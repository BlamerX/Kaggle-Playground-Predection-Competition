"""
S6E5 V3 - LightGBM (GPU)
================================================================================
Strategy: LightGBM with Feature Engineering + Original Data (per-fold concat)

Key Techniques:
1. LGBMClassifier via lightgbm with GPU acceleration
...
LightGBM Configuration (aligned with proven S6E4 params):
...
# =============================================================================
# LGBMClassifier: GPU-accelerated gradient boosting
#
...
#   num_leaves=32: moderate leaf count (2^depth default for depth=4)
#   learning_rate=0.05: conservative LR for stability
#   device='cuda': GPU acceleration
#   feature_fraction=0.6: lower feature subsampling for regularization
#   bagging_fraction=0.7: moderate row subsampling (bagging)
#   min_child_samples=12: regularize leaf nodes
#   lambda_l1=10, lambda_l2=10: strong L1/L2 regularization
#   NO max_bin: CRITICAL -- setting max_bin manually causes fatal
#               "bin size X cannot run on GPU" errors on LightGBM GPU
...
def feature_engineering(df, cat_cols, num_cols, category_map, fit=False):
    ...
    for col in num_cols + cat_from_num_cols:
        ...
        if fit:
            ...
            codes = np.floor(df[col]).map(code_map).fillna(len(uniques)).astype('int32')
        df[cat_name] = codes
    ...
    for cols in important_combos:
        ...
        if fit:
            ...
            code_map = {cat: i for i, cat in enumerate(uniques)}
            codes = combo_series.map(code_map).fillna(len(uniques)).astype('int32')
        df[combo_name] = codes
...
if __name__ == "__main__":
    ...
    # Label-encode base string categoricals to float32
    # NOTE: All features must be float32 for LightGBM GPU -- int32 columns get
    # auto-detected as categorical, causing "bin size X cannot run on GPU" errors.
    # The ordinal encoding is preserved as float values (0.0, 1.0, 2.0, etc.)
    for col in cat_cols:
        if col not in new_cat_cols:
            ...
            le.fit(combined)
            X[col]      = le.transform(X[col].astype(str)).astype('float32')
            X_test[col] = le.transform(X_test[col].astype(str)).astype('float32')
            orig[col]   = le.transform(orig[col].astype(str)).astype('float32')

    # New categorical features -> float32 (not int32, prevents GPU bin overflow)
    ...
    print(f"   Done: {len(cat_cols)} cat (float32) + {len(num_cols)} num (float32)")

    # =========================================================================
    # [3/5] TRAINING (Per-fold: concat orig -> TE -> LightGBM)
    # =========================================================================
    print(f"\n[3/5] Training LightGBM ({CFG.N_FOLDS}-Fold CV, orig concat)...")
    ...
    # =========================================================================
    print(f"\n{'=' * 80}")
    print(f"V3 RESULTS - LightGBM ({CFG.DEVICE})")
    print(f"{'=' * 80}")
