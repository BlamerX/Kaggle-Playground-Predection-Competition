"""
S6E5 V8 - PyTabKit FTTransformer (GPU)
================================================================================
Strategy: FTTransformer (self-attention on tabular data) with V7's proven
FE pipeline (45 features) on 10-fold CV.

FTTransformer applies multi-head self-attention to each feature independently
(via learned embeddings), making it fundamentally different from tree-based
models (XGB/LGBM/CB) and dense NN models (RealMLP). This architectural
diversity adds value for the hill climber even if standalone AUC doesn't
beat the best tree model.
...
# Auto-install pytabkit + skorch (FTTransformer uses skorch backend)
...
class CFG:
    ...
    # Model params — FTTransformer (skorch backend)
...
if __name__ == "__main__":
    ...
    print(f"Model: FTTransformer (self-attention on tabular data)")
    ...
    # =========================================================================
    # [3/5] TRAINING — FTTransformer 10-Fold CV
    # =========================================================================
    print(f"\n[3/5] Training FTTransformer ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, ((tr_idx, val_idx), (or_tr_idx, or_val_idx)) in enumerate(
        ...
        print(f"\n{'#' * 16}")
        print(f"### Fold {fold}/{CFG.N_FOLDS} [FTTransformer] ...")
        print(f"{'#' * 16}")
    ...
    # =========================================================================
    print(f"\n{'=' * 80}")
    print(f"V8 RESULTS - FTTransformer ({CFG.DEVICE})")
    print(f"{'=' * 80}")
