"""
S6E5 V16 - Extra Trees Classifier
================================================================================
Strategy: Bagged trees with extreme randomization for diversity

ExtraTrees Architecture:
  - Ensemble of 1000 extremely randomized decision trees
  - Splits chosen at random thresholds (not optimal like RF)
  - max_features='sqrt' for feature subsampling
  - class_weight='balanced' for 80/20 imbalance

Why ExtraTrees adds diversity for hill climber:
  - More random than RandomForest (random thresholds, not optimal)
  - Different split selection creates different bias/variance tradeoff
  - Not gradient-boosted like XGBoost/LightGBM/CatBoost
  - Complementary error patterns with bagging + extreme randomization

Feature Engineering: V1 pipeline (identical to V1/V14/V15 for fair comparison)
...
class CFG:
    VERSION_NAME = "v16"
    EXP_ID = "S6E5_V16_ExtraTrees"
    DEVICE = "cpu"  # sklearn, no GPU needed
...
# =============================================================================
# 3. MODEL PARAMETERS (ExtraTrees)
# =============================================================================
# ExtraTreesClassifier: Extremely Randomized Trees
#
# Key differences from RandomForest (V12):
#   - Splits chosen at RANDOM thresholds (not best among candidates)
#   - No bootstrapping by default (uses full dataset)
#   - Lower variance, potentially higher bias
#   - Much faster training (no threshold search per split)
#
# Hyperparameter choices:
#   - n_estimators=1000: large ensemble for stable predictions
#   - max_depth=None: no depth limit (ExtraTrees benefits from deep trees)
#   - min_samples_split=10: regularization against overfitting
#   - min_samples_leaf=5: smooth leaf predictions
#   - max_features='sqrt': random subset of features per split (decorrelation)
...
# =============================================================================
# 4. FEATURE ENGINEERING (V1 pipeline — identical to V1/V14/V15)
# =============================================================================
def feature_engineering(df, cat_cols, num_cols, category_map, fit=False):
    """
    FE pipeline: 14 raw features -> 38 features (same as V1/V14/V15).
...
if __name__ == "__main__":
    ...
    print(f"Strategy: ExtraTreesClassifier (n_estimators={MODEL_PARAMS['n_estimators']}) + V1 FE")
    print("=" * 80)
    ...
    # =========================================================================
    # [3/5] TRAINING (Per-fold: concat orig -> TE -> ExtraTreesClassifier)
    # =========================================================================
    print(f"\n[3/5] Training ExtraTreesClassifier "
    ...
    for fold, ((tr_idx, val_idx), (or_tr_idx, or_val_idx)) in enumerate(
        ...
        # ---- FIT EXTRATREES CLASSIFIER ----
        model = ExtraTreesClassifier(**MODEL_PARAMS)

        print(f"   Fitting ExtraTreesClassifier "
              f"(n_estimators={MODEL_PARAMS['n_estimators']}, "
        ...
        tst_preds = model.predict_proba(X_tst[all_features])[:, 1]
    ...
    # =========================================================================
    print(f"\n{'=' * 80}")
    print(f"V16 RESULTS - ExtraTreesClassifier ({CFG.DEVICE})")
    print(f"{'=' * 80}")
    ...
    print(f"Model config: ExtraTreesClassifier("
          f"n_estimators={MODEL_PARAMS['n_estimators']}, "
          f"max_depth={MODEL_PARAMS['max_depth']}, "
          f"min_samples_split={MODEL_PARAMS['min_samples_split']}, "
