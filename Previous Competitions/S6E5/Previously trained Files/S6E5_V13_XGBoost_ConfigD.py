"""
S6E5 V13 - XGBoost Lossguide (GPU) with Config D Features
================================================================================
Strategy: V7 pipeline + Config D features (ablation winner, +0.00080 local)

Ablation Study Results (10% sample, 3-fold, HistGBM, full V7 baseline):
  Config D (TyreLife_sq REPLACE + Safe3): +0.00080 (WINNER)
  Safe3 = Degradation_Rate + RaceProgress_x_TyreLife + Compound_Stint_

Config D Features:
  1. TyreLife_sq = TyreLife^2 (REPLACE TyreLife — tree approximates x via splits)
...
Changes from V7:
  - REPLACE TyreLife with TyreLife_sq (non-linear tire cliff effect)
  - ADD Degradation_Rate (per-lap degradation speed — top notebook feature)
  - ADD RaceProgress_x_TyreLife (race phase x tire age interaction)
  - ADD Compound_Stint_ (MEDIUM Stint 2 = 44.8% pit rate — from ferariz)
  - DROP TyreLife and TyreLife_cat_ (replaced by TyreLife_sq)

Feature Set (~49 features = V7's 47 - 1 TyreLife - 1 TyreLife_cat_ + 4 new):
  - 13 base features (3 cat + 10 num, TyreLife REMOVED)
  ...
  - 1 interaction category: Compound_Stint_ (NEW)
  - Per-fold TE on interaction categories: 3 TE features (1 NEW)
...
class CFG:
    VERSION_NAME = "v13"
    EXP_ID = "S6E5_V13_XGBoost_Lossguide_ConfigD"
    DEVICE = "cuda"
...
# =============================================================================
# 4. MODEL PARAMETERS (lossguide leaf-wise — same as V7)
# =============================================================================
# Same proven lossguide params from V7.
# Key params:
...
# =============================================================================
# 5. FEATURE ENGINEERING (V7 pipeline + Config D features)
# =============================================================================
...
if __name__ == "__main__":
    ...
    print(f"Strategy: V7 FE + Config D features (ablation winner +0.00080)")
    ...
    # =========================================================================
    # [2/5] FEATURE ENGINEERING (14 raw -> ~44 features, V7 + Config D)
    # =========================================================================
    print(f"\n[2/5] Feature Engineering (V7 + Config D)...")
    ...
    print(f"\n[3/5] Training XGBoost "
          f"({CFG.N_FOLDS}-Fold CV, orig concat, lossguide, Config D)...")
    ...
    for fold, ((tr_idx, val_idx), (or_tr_idx, or_val_idx)) in enumerate(
        ...
        if fold == 1:
            ...
            print(f"     Global (V7+ConfigD): {len(list(X.columns))}")
    ...
    # =========================================================================
    print(f"\n{'=' * 80}")
    print(f"V13 RESULTS - XGBoost Lossguide + Config D ({CFG.DEVICE})")
    print(f"{'=' * 80}")
