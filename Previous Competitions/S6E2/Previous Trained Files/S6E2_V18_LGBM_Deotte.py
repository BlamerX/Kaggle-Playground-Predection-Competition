...
class CFG:
    VERSION = "V18"
    DESCRIPTION = "LGBM_Deotte_Clone"
    
    # LGBM Adaptation of Deotte Params
    # XGB: depth=3, lr=0.0025, subsample=0.8
    # LGBM: num_leaves=8 (approx depth 3), lr=0.005, feature_fraction=0.8
    LGBM_PARAMS = {
        'n_estimators': 50000,
        'learning_rate': 0.005,
        'num_leaves': 8, # 2^3 = 8
        'subsample': 0.8,
        'bagging_freq': 1, # Enable subsampling
        'colsample_bytree': 0.8,
        'random_state': 42,
        'min_child_samples': 20, # Constraint
        'objective': 'binary',
        'metric': 'auc',
...
def main():
    ...
    for i, (train_index, val_index) in enumerate(kf.split(train)):
        ...
        # Final Prep
        # LightGBM: DROP CATEGORICALS (using TE instead)
        # Even though standard LGBM handles cats well, we are testing strict Deotte clone here.
        # So we drop TO_REMOVE.
        
        current_cols = X_train.columns.tolist() # snapshot
        ...
        # Convert to Numpy to avoid cudf.pandas Proxy issues with LightGBM
        # LightGBM may struggle with the proxy object or invalid pointers
        print(f"  [Fold {i+1}] Inner Feature Engineering Complete. Converting to Numpy...")
        ...
        # Train
        print(f"  [Fold {i+1}] Starting LGBM Training...")
        ...
        # Predict
        val_p = model.predict_proba(X_val_np)[:,1]
        oof[val_index] = val_p
