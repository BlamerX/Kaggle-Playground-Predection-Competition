...
class CFG:
    VERSION = "V17"
    DESCRIPTION = "CatBoost_Deotte_Clone"
    
    # CatBoost Adaptation of Deotte Params
    # XGB: depth=3, lr=0.0025, subsample=0.8
    # Cat: depth=3, lr=0.0025, subsample=0.8
    CAT_PARAMS = {
        'iterations': 50000,
        'learning_rate': 0.0025,
        'depth': 3,
        'subsample': 0.8,
        # 'colsample_bylevel': 0.8, # CAUSES GPU ERROR (pairwise only)
        'random_seed': 42,
...
def main():
    ...
    for i, (train_index, val_index) in enumerate(kf.split(train)):
        ...
        # So we do NOT convert to category type for CatBoost, we simply DROP them as planned.
        # This keeps the strategy pure: CatBoost sees exactly what XGB saw (Numerics + TE).
