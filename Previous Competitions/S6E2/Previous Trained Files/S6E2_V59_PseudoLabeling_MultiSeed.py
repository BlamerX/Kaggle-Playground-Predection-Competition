...
class CFG:
    VERSION = "V59"
    DESCRIPTION = "RealMLP_Distillation_MultiSeed"

    # RealMLP_TD_Classifier Params (Same as V58/V51)
    PARAM_GRID = {
        'device': DEVICE,
        'random_state': 42, # Will be overwritten by loop
        'verbosity': 0,     # Reduce verbosity for multi-seed
        'n_epochs': 100,
        'batch_size': 256,
        'n_ens': 1, 
...
def main():
    ...
    # 4. Multi-Seed CV Training (Fixed Folds for valid OOF)
    ...
    # FIXED FOLDS (Seed 42) - Essential for OOF stacking compatibility
    skf = StratifiedKFold(n_splits=CFG.N_FOLDS, shuffle=True, random_state=42) 
    fold_indices = list(skf.split(X_orig, y_orig))
    
    print(f"\nStarting Multi-Seed Training ({len(CFG.SEEDS)} Seeds) with FIXED FOLDS...")

    
    for seed_idx, seed in enumerate(CFG.SEEDS):
        print(f"\n⚡ RE-STARTING SEED {seed} ({seed_idx + 1}/{len(CFG.SEEDS)}) [FIXED FOLDS] ⚡")
