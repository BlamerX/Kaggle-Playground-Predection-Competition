...
"""
S6E1 Stage 3 - LightGBM Hybrid Model (V32 Architecture + V35 Params)
====================================================================
Features: Hybrid V32 + Golden Features (from s6e1_stage3_model_training.py)
Model: LightGBM (Params from v35_lgbm.py)
Architecture: 2-Stage Stacking (Ridge -> LightGBM)
"""
...
# ============================================================================
# 1. DATA LOADING (V32 Style)
# ============================================================================

print("="*80)
print("S6E1 Stage 3 - LightGBM Hybrid Model")
print("="*80)
...
# ============================================================================
# 3. FEATURE ENGINEERING (Hybrid V32 + Golden)
# ============================================================================
...
# LightGBM handles categories natively if they are dtype 'category'
# CRITICAL FIX: We must concatenate ALL data to ensure category codes/dictionaries match exactly.
print("Concatenating for global categorical casting...")
n_train = len(X_lgb)
...
# FIX: Switch to CPU to allow high-cardinality categorical features (like class_attendance).
# GPU is limited to 255 bins, which forces us to treat predictors as float, hurting OOF (8.71 vs 8.60).
print("Casting categorical features (Global - All Base Features)...")
...
print(f"Global casting complete. Train shape: {X_lgb.shape}")

print(f"Final feature count: {X_lgb.shape[1]}")
...
# Validated Parameters with Categorical Tuning (CPU Optimized)
lgb_params = {
    'objective': 'regression',
    'metric': 'rmse',
    'n_estimators': 20000,
    'max_depth': 12,        # Increased from 8 to allow more capacity with 500 leaves
    'num_leaves': 128,      # Reduced from 500 to prevent overfitting
    'learning_rate': 0.015, # Slower learning for better convergence
    'subsample': 0.7,       # Slightly more randomness
    'colsample_bytree': 0.7,
    'reg_alpha': 0.01,
    'reg_lambda': 1.0,
    'min_data_in_leaf': 50, # Prevent overfitting in leaves
    'cat_smooth': 30,       # CRITICAL: Smoothing for high-cardinality categories (class_attendance)
    'cat_l2': 10,           # CRITICAL: L2 regularization for categories
    'max_bin': 1023,
    'device': 'cpu',
    'verbose': -1,
    'n_jobs': -1
...
def train_lgb_seed(seed):
    ...
    test_predictions = []
    oof_predictions = np.zeros(len(X_lgb), dtype=float)
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_lgb, y), start=1):
        ...
        test_predictions.append(model.predict(X_test_lgb))
    
    oof_rmse = np.sqrt(mean_squared_error(y, oof_predictions))
    print(f"  seed={seed} OOF RMSE: {oof_rmse:.5f}")
    
    return oof_predictions, np.mean(test_predictions, axis=0)

oof_total = np.zeros(len(X_lgb))
test_total = np.zeros(len(X_test_lgb))

print(f"Starting {len(SEEDS)}-seed Training Loop...")
for seed in SEEDS:
    seed_oof, seed_test = train_lgb_seed(seed)
...
print(f"\n{'='*80}")
print(f"FINAL 5-SEED LIGHTGBM OOF RMSE: {final_rmse:.5f}")
print("="*80)

submission = pd.read_csv(test_file, usecols=['id'])
submission['exam_score'] = test_total
submission.to_csv("submission_stage3_lgb.csv", index=False)

oof_df = pd.DataFrame({'id': train_df['id'], 'exam_score': oof_total})
oof_df.to_csv("oof_stage3_lgb.csv", index=False)

print("Saved files: submission_stage3_lgb.csv, oof_stage3_lgb.csv")
