"""
S6E1 V1 ULTIMATE - 5-Seed Averaging + 10-Fold CV
=================================================
Based on V32 exact FE (proven best)
...
print("="*80)
print("S6E1 V1 ULTIMATE - 5-Seed Averaging + 10-Fold")
print("="*80)
...
print(f"  Folds: {FOLDS}")
print(f"  Seeds: {SEEDS}")
print(f"  Total models: {FOLDS * N_SEEDS} = {FOLDS} folds × {N_SEEDS} seeds")
...
# ============================================================================
# 7. XGB 5-SEED AVERAGING (10-fold each)
# ============================================================================

print(f"\n{'='*80}")
print("XGBOOST 5-SEED AVERAGING (10-fold × 5 seeds)")
print("="*80)
...
for seed_idx, seed in enumerate(SEEDS, start=1):
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_xgb, y), start=1):
        ...
        print(f"  Fold {fold:2d}: {fold_rmse:.5f} | Best: {model.best_iteration}")
...
print(f"\nIndividual seed mean: {np.mean(seed_rmses):.5f}")
print(f"5-Seed averaged:     {final_oof_rmse:.5f}")
print(f"Improvement:         {np.mean(seed_rmses) - final_oof_rmse:+.5f}")
...
Configuration:
  Folds: {FOLDS}
  Seeds: {SEEDS}
  Total XGB models: {FOLDS * N_SEEDS}

Results:
  Ridge OOF RMSE:     {ridge_rmse:.5f}
  XGB OOF RMSE (avg): {final_oof_rmse:.5f}
  Features:           {X_xgb.shape[1]}
""")
...
oof_df = pd.DataFrame({ID_COL: train_df[ID_COL], TARGET: final_oof})
oof_df.to_csv("oof_v1.csv", index=False)

print("✓ submission_v1.csv (5-seed averaged)")
print("✓ oof_v1.csv (5-seed averaged)")

print(f"\n{'='*80}")
print("V1 ULTIMATE COMPLETE")
print("="*80)
