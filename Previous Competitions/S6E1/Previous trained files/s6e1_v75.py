"""
S6E1 V75 - CatBoost with TabM Baseline (Using V61 OOF)
=======================================================
OOF-Leveraged: Uses V61 TabM predictions as CatBoost baseline.
...
V61 is BEST SINGLE MODEL at 8.56152 LB!
Expected: Could beat V73 XGB (8.56137) to become NEW BEST!
"""
...
class CFG:
    EXP_ID = "V75_CatBoost_TabM_Baseline"
    SEED = 42
    N_FOLDS = 10
    TARGET = 'exam_score'
...
print("="*80)
print("S6E1 V75 - CatBoost with TabM Baseline (V61 OOF)")
print("="*80)
print("⚡ Using V61 OOF as CatBoost baseline (OOF-leveraged approach)")
print("📚 Source: S5E10 1st Place Writeup")
print("🏆 V61 is BEST SINGLE MODEL at 8.56152 LB!")
...
# Create baseline for original data (use mean of V61 predictions as proxy)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X), 1):
    ...
    train_pool = Pool(
        data=X_tr_aug, 
        label=y_tr_aug, 
        baseline=baseline_tr_aug,  # V61 TabM predictions as baseline
        cat_features=CAT_COLS
    )
...
print(f"| **V75** | **CatBoost + TabM Baseline** | **{final_rmse:.5f}** | {improvement:+.5f} |")

if improvement > 0:
    print(f"\n🏆 V75 IMPROVED by {improvement:.5f}!")
else:
    print(f"\n⚠️ V75 was {-improvement:.5f} worse than baseline.")
...
print(f"| V61 (Best TabM) | 8.56152 |")
print(f"| V70 (Best FTT) | 8.56168 |")
print(f"| V58 (CatBoost+FTT) | 8.56168 |")
print(f"| **V75 (CatBoost+TabM)** | **??? (submit!)** |")
...
submission.to_csv("submission_v75.csv", index=False)

oof_df = pd.DataFrame({'id': train_df['id'], 'exam_score': oof_preds})
oof_df.to_csv("oof_v75.csv", index=False)

elapsed = (time.time() - start_time) / 60
print(f"\nFiles saved:")
print(f"  submission_v75.csv")
print(f"  oof_v75.csv")
print(f"\nTotal time: {elapsed:.1f} minutes")

print("\n" + "="*80)
print("✅ V75 Complete! Submit to see if it beats V73 (8.56137)!")
print("="*80)
