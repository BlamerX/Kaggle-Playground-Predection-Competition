"""
S6E1 V20 - EDA-Inspired Improvements
=====================================
Based on V16 with these tests:
- Test A: study_method ordinal encoding by target mean
- Test B: Prediction clipping to [19.6, 100] (Tobit model bounds)

Baseline: V16 = 8.60770 OOF, 8.56513 LB
"""
...
print("=" * 80)
print("S6E1 V20 - EDA-Inspired Improvements")
print("=" * 80)
...
# V20 IMPROVEMENT: Tobit model censoring bounds
CLIP_MIN = 19.6  # From Tobit analysis: y_min = 19.599
CLIP_MAX = 100   # From Tobit analysis: y_max = 100

print(f"Train: {train_df.shape}, Test: {test_df.shape}, Original: {original_df.shape}")
print(f"V20: Clipping predictions to [{CLIP_MIN}, {CLIP_MAX}]")

# ============================================================================
# Feature Engineering (V13 + V20 improvements)
# ============================================================================
def preprocess_v20(df):
    """V13 features + V20 improvements."""
    df_temp = df.copy()
    eps = 1e-5
    ...
    # V20 IMPROVEMENT: study_method ordinal by target mean
    # From EDA: coaching(69.3) > mixed(65.1) > group(60.5) > online(59.7) > self-study(57.7)
    ...
    # Ordinal × numeric (including new study_method)
    ...
    # V20: study_method × study_hours interaction
...
# ============================================================================
print("\n--- Feature Engineering ---")
X_raw, numeric_cols = preprocess_v20(train_df)
X_test_raw, _ = preprocess_v20(test_df)
X_orig_raw, _ = preprocess_v20(original_df)
...
print(f"Features: {X.shape[1]} (V16 was 45, V20 has {X.shape[1]})")
...
oof_df.to_csv("oof_v20.csv", index=False)

# Submission
submission_df[TARGET] = np.mean(test_preds_xgb, axis=0)
submission_df.to_csv("submission_v20.csv", index=False)

print(f"  oof_v20.csv")
print(f"  submission_v20.csv")

# ============================================================================
# Final Summary
# ============================================================================
print("\n" + "=" * 80)
print("V20 SUMMARY (EDA-Inspired Improvements)")
print("=" * 80)
print(f"  Ridge OOF:   {ridge_oof:.5f}")
print(f"  XGBoost OOF: {xgb_oof:.5f}")
print(f"  V16 Baseline: 8.60770")
print(f"  Delta:       {xgb_oof - 8.60770:+.5f}")
print(f"  Total Time:  {(ridge_time + xgb_time)/60:.1f} min")
print("=" * 80)
print("\nV20 Changes:")
print("  1. study_method ordinal by target mean: coaching(4) > self-study(0)")
print("  2. study_method × study_hours interaction feature")
print("  3. Prediction clipping to [19.6, 100] (Tobit bounds)")
