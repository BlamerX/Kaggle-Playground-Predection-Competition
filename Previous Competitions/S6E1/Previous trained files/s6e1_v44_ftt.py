"""
S6E1 V44 - FT-Transformer WITHOUT Golden Features
==================================================
Based on S3_FTT but using V28 feature engineering (NO Golden Features).
Expected: Better LB generalization than S3_FTT (8.56379)
"""
...
print("\n" + "="*80)
print("S6E1 V44 - FT-Transformer (NO Golden Features)")
print("="*80)
...
for seed in CFG.SEEDS:
    ...
    # FTT Parameters (Same as Stage 3)
    model_params = {
        'device': CFG.DEVICE,
        'random_state': seed,
        'verbosity': 0,
...
print(f"\n{'='*80}")
print(f"V44 FTT (No Golden) OOF RMSE: {final_rmse:.5f}")
print(f"S3 FTT (With Golden) OOF RMSE: 8.60462")
print(f"Improvement: {8.60462 - final_rmse:+.5f}")
print("="*80)

# Save
submission = test_df[['id']].copy()
submission['exam_score'] = avg_test
submission.to_csv("submission_v44_ftt.csv", index=False)

oof_df = pd.DataFrame({'id': train_df['id'], 'oof_pred': avg_oof})
oof_df.to_csv("oof_v44_ftt.csv", index=False)

print("\nSaved: submission_v44_ftt.csv, oof_v44_ftt.csv")
