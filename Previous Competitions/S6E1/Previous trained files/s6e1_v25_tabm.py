"""
S6E1 V25 - TabM (more_capacity)
===============================
NEW BEST: 8.56226 LB (beats V24's 8.56241)

Key Parameters:
- tabm_k=32 (vs V24's 24)
- d_embedding=24 (vs V24's 16)
- dropout=0.11 (same as V24)

This script is the production version with the winning hyperparameters
from the V26 hyperparameter sweep.
"""
...
class CFG:
    EXP_ID = "V25_TabM_more_capacity"
    SEED = 42
    N_FOLDS = 10
    TARGET = 'exam_score'
    DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
    
    # TabM Params (Winning Config from Sweep)
    BATCH_SIZE = 256
    EPOCHS = 100
    LR = 1e-3
    TABM_K = 32           # Increased from V24's 24
    D_EMBEDDING = 24      # Increased from V24's 16
    DROPOUT = 0.11
...
print("\n" + "="*80)
print("S6E1 V25 - TabM (more_capacity)")
print("="*80)
...
# ============================================================================
# 5. MODEL TRAINING (TabM with more_capacity config)
# ============================================================================

print("\nTraining TabM (10-Fold, more_capacity config)...")
...
print(f"\nAverage Fold RMSE: {np.mean(fold_scores):.5f}")
print(f"OOF RMSE:          {oof_rmse:.5f}")
print(f"V24 Baseline:      8.60648")
print(f"Delta:             {oof_rmse - 8.60648:+.5f}")

# Save OOF
oof_df = pd.DataFrame({'id': train_df['id'], 'exam_score': oof_predictions})
oof_df.to_csv('oof_v25_tabm.csv', index=False)

# Save Submission
avg_test_preds = np.mean(test_predictions, axis=0)
submission_df['exam_score'] = avg_test_preds
submission_df.to_csv('submission_v25_tabm.csv', index=False)

print("\nSaved: oof_v25_tabm.csv, submission_v25_tabm.csv")
print(f"\n{'='*80}")
print(f"V25 COMPLETE - Expected LB: ~8.56226 (NEW BEST)")
print("="*80)
