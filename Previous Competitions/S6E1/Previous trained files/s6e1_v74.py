"""
S6E1 V74 - LightGBM + Boosted Pseudo-Labels (Using V67 OOF)
============================================================
Uses V67 OOF (best LGB) instead of V46 for better baseline!

Strategy:
1. LOAD V67 OOF + V67 submission (best LGB model)
2. Calculate residuals = y_true - V67_oof
3. Train residual LGB model (improved config)
4. Update pseudo-labels: new = old + α × residual_pred
5. Retrain LGB with updated pseudo-labels

Expected: Beat V67's 8.57986 LB!
"""
...
class CFG:
    EXP_ID = "V74_LGB_BoostedPL_V67OOF"
    N_FOLDS = 10
    TARGET = "exam_score"
    ALPHA = 0.15  # Increased from 0.1

print("="*80)
print("S6E1 V74 - LightGBM + Boosted Pseudo-Labels (Using V67 OOF)")
print("="*80)
print("⚡ Using V67 OOF (best LGB) - should beat 8.57986!")
...
if os.path.exists('/kaggle/input/playground-series-s6e1/train.csv'):
    ...
    oof_path = "/kaggle/input/oof-and-submission/Season6episode1/Previous trained files/OOF/oof_v67.csv"
    sub_path = "/kaggle/input/oof-and-submission/Season6episode1/Previous trained files/Submissions/submission_v67.csv"
else:
    print("Environment: LOCAL")
    train_file = "Dataset/train.csv"
    test_file = "Dataset/test.csv"
    original_file = "Dataset/Exam_Score_Prediction.csv"
    oof_path = "Previous trained files/OOF/oof_v67.csv"
    sub_path = "Previous trained files/Submissions/submission_v67.csv"

train_df = pd.read_csv(train_file)
test_df = pd.read_csv(test_file)

if os.path.exists(original_file if not os.path.exists('/kaggle/input') else original_file):
...
# ============================================================================
# 3. LOAD EXISTING V67 OOF & SUBMISSIONS
# ============================================================================

print("\n" + "="*80 + "\nLOADING V67 OOF (BEST LGB MODEL!)\n" + "="*80)

v67_oof = pd.read_csv(oof_path)
v67_sub = pd.read_csv(sub_path)

print(f"✓ Loaded V67 OOF: {v67_oof.shape}")
print(f"✓ Loaded V67 submission: {v67_sub.shape}")

oof_col = 'oof_pred' if 'oof_pred' in v67_oof.columns else 'exam_score'
oof_baseline = v67_oof[oof_col].values
test_pseudo_labels = v67_sub['exam_score'].values

y = train_df[TARGET].values

baseline_rmse = np.sqrt(mean_squared_error(y, oof_baseline))
print(f"\nV67 Baseline OOF RMSE: {baseline_rmse:.5f}")
print("V67 LB Score: 8.57986 (best LGB)")

train_residuals = y - oof_baseline
print(f"Residual stats: mean={train_residuals.mean():.4f}, std={train_residuals.std():.4f}")

# ============================================================================
# 4. CATEGORY MEAN TRANSFORMER (CMT)
# ============================================================================
...
# ============================================================================
# 8. BOOSTED PSEUDO-LABELS
# ============================================================================

print(f"\n{'='*80}")
print("BOOSTED PSEUDO-LABELS (Using V67 as baseline)")
print("="*80)
...
res_lgb_params = {
    ...
    'max_depth': 8,
    'num_leaves': 64,
    'learning_rate': 0.01,  # Lower for residuals
    'subsample': 0.7,
    'colsample_bytree': 0.7,
    'reg_alpha': 0.1,
    'reg_lambda': 2.0,
...
for fold, (train_index, val_index) in enumerate(kf.split(X_lgb, y), start=1):
    ...
    print(f"  Residual Fold {fold}: done (best_iter={res_model.best_iteration_})")
...
print(f"\nPseudo-labels updated (α={CFG.ALPHA})")
...
print(f"\nOOF RMSE: {updated_rmse:.5f} (vs V67 baseline: {improvement:+.5f})")
...
submission.to_csv("submission_v74.csv", index=False)

oof_df = pd.DataFrame({'id': train_df['id'], 'exam_score': oof_updated})
oof_df.to_csv("oof_v74.csv", index=False)

elapsed = (time.time() - start_time) / 60
print(f"\nFiles saved:")
print(f"  submission_v74.csv")
print(f"  oof_v74.csv")
...
print(f"|---------|-------|----------|----------|")
print(f"| V67 | LGB + PL (baseline) | {baseline_rmse:.5f} | 8.57986 |")
print(f"| **V74** | **LGB + PL (V67 OOF)** | **{updated_rmse:.5f}** | **~8.57-8.58** |")
print(f"\nTarget: Beat 8.57986!")
print("\n✅ V74 ready for submission!")
print("="*80)
