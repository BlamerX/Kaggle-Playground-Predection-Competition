"""
S6E1 V120 - CatBoost Lossguide (CPU Mode)
==========================================
Base: V110 (8.54708 LB - BEST)
New: 
  1. grow_policy='Lossguide' (requires CPU)
  2. high_study threshold at 3.9
  3. Single seed (to reduce CPU time)

Note: Lossguide is NOT GPU compatible, so this runs on CPU (slower)
Estimated time: ~60-90 min (1 seed × 10 folds × 2 phases)
"""
...
print("="*80)
print("S6E1 V120 - CatBoost Lossguide (CPU Mode)")
print("="*80)
...
print(f"\n*** V120 Changes ***")
print(f"  1. grow_policy: SymmetricTree → Lossguide")
print(f"  2. task_type: GPU → CPU (required for Lossguide)")
print(f"  3. high_study threshold: 7.0 → 3.9")
print(f"  4. Single seed (to reduce CPU time)")

# ============================================================================
# V120: CatBoost Lossguide (CPU)
# ============================================================================

print(f"\n{'='*80}")
print("V120: CATBOOST LOSSGUIDE (CPU)")
print("="*80)

SEED = 42  # Single seed to reduce CPU time
N_FOLDS = 10
ALPHA = 0.1
...
# V120 PARAMS: Lossguide on CPU
catboost_v120_params = {
    'iterations': 3000,                 # Reduced from 5000 for CPU speed
    'learning_rate': 0.03,              # Slightly higher for fewer iterations
    'depth': 6,
    'l2_leaf_reg': 3,
    'grow_policy': 'Lossguide',         # KEY: Lossguide (CPU only)
    'max_leaves': 31,                   # Required with Lossguide
    'task_type': 'CPU',                 # Required for Lossguide
...
print(f"\n--- Phase 1: Training on residuals (Seed {SEED}) ---")
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train), start=1):
    ...
    model = CatBoostRegressor(**catboost_v120_params)
    ...
    print(f"  Fold {fold}/{N_FOLDS}: val_rmse={val_rmse:.5f}, time={fold_time:.1f} min")

phase1_time = (time.time() - start_time) / 60
print(f"\nPhase 1 complete: {phase1_time:.1f} min")
...
final_oof_v120 = np.clip(v77_train + oof_final, 0, 100)
final_test_v120 = np.clip(v77_test + np.mean(test_final, axis=0), 0, 100)
v120_rmse = np.sqrt(mean_squared_error(y, final_oof_v120))
...
|---------|-------|---------|----------|----------|
| V110 | DART + 5-seed (GPU) | Baseline | 8.55927 | 8.54708 |
| **V120** | **Lossguide (CPU)** | **+Lossguide, +Study3.9** | **{v120_rmse:.5f}** | **?** |
""")

improvement = 8.55927 - v120_rmse
print(f"OOF Improvement vs V110: {improvement:+.5f}")

# ============================================================================
# SAVE
# ============================================================================

pd.DataFrame({'id': test_df['id'], 'exam_score': final_test_v120}).to_csv("submission_v120.csv", index=False)
pd.DataFrame({'id': train_df['id'], 'exam_score': final_oof_v120}).to_csv("oof_v120.csv", index=False)

elapsed = (time.time() - start_time) / 60
print(f"\nFiles saved: submission_v120.csv, oof_v120.csv")
