"""
S6E3 V38 - TabM with V16 Features + Hidden Features
================================================================================
Strategy: V21 TabM (LB 0.91682) + Hidden Features from V36

V21 TabM used V16 features (digit + n-gram TE).
V38 adds the hidden features that helped V36:
  - fiber_m2m: THE KILLER FEATURE (55% vs 6.8% churn)
  - combined_risk, risk_score, contract_risk, payment_risk, internet_risk
  - is_brand_new, stream_no_prot

Hypothesis: Hidden features that helped XGBoost may also help TabM's 
neural architecture learn better representations.

Architecture: IDENTICAL to V21 (proven best NN)
...
class CFG:
    VERSION       = "v38"
    EXP_ID        = "S6E3_V38_TabM_V16_HiddenFeatures"
    ...
    SEED          = 42
    N_FOLDS       = 10        # Same as V21
    INNER_FOLDS   = 5

    # V21 proven TabM hyperparameters (UNCHANGED)
    TABM_PARAMS = {
...
def main():
    ...
    print("=" * 80)
    print("\nV38 = V21 TabM + V36 Hidden Features")
    print("  [SAME] TabM architecture (tabm-mini-normal, k=32, pwl embeddings)")
    print("  [SAME] V16 features (digit + n-gram TE)")
    print("  [NEW] 8 Hidden Features from V36:")
    print("        - fiber_m2m, combined_risk, risk_score")
    print("        - contract_risk, payment_risk, internet_risk")
    ...
    print("\n[2/6] Feature Engineering (V16 pipeline)...")
    ...
    # ── [NEW] Hidden Features (V36) ─────────────────────────────────────
    print("\n[4/6] Hidden Features (from V36)...")
    HIDDEN_FEATURES = [
    ...
    for df in [train, test]:
        ...
        # 6. STREAMING WITHOUT PROTECTION
    ...
    print(f"  ✅ {len(HIDDEN_FEATURES)} hidden features added")

    # ── N-gram Categorical columns (V16 Top-6) ──────────────────────────
    print("\n[5/6] N-gram Categorical Features (V16 Top-6)...")
    ...
    # ── [6/6] Training (10-Fold CV) ───────────────────────────────────────────
    print(f"\n[6/6] Training TabM ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold_i, (train_idx, val_idx) in enumerate(skf.split(train, y_all)):
        ...
        # V21 reference folds (for live comparison)
    ...
    V21_OOF     = 0.91898   # V21 TabM reference OOF (10-fold)

    print(f"\n{'='*80}")
    print(f"V38 RESULTS — TabM with V16 + Hidden Features")
    print(f"{'='*80}")
    print(f"Overall CV AUC : {overall_auc:.5f}  (Mean: {mean_score:.5f} +/- {std_score:.5f})")
    print(f"V21 TabM Baseline: {V21_OOF:.5f}  (10-Fold OOF, V16 features)")
    print(f"Delta vs V21    : {overall_auc - V21_OOF:+.5f}")
    print(f"Per-fold: {' | '.join(f'{s:.5f}' for s in fold_scores)}")

    verdict = ("🏆 IMPROVED"  if overall_auc > V21_OOF + 0.00010 else
               "✅ MARGINAL"  if overall_auc > V21_OOF else
               "= SAME"       if abs(overall_auc - V21_OOF) < 0.00005 else
               "❌ WORSE")
