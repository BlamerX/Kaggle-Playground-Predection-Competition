"""
S6E3 V60 - Tabular ResNet
================================================================================
Strategy: ResNet-style skip connections for tabular data

Key Idea:
  Apply ResNet architecture (skip connections) to tabular data.
  Skip connections allow training deeper networks by mitigating vanishing gradients.
  Each residual block: output = F(x) + x (identity shortcut)

Reference: V21 TabM (OOF: 0.91898, LB: 0.91682)

Architecture:
  Input Layer: input_dim -> hidden_dim
  Residual Block 1: hidden_dim -> hidden_dim (with skip connection)
  Residual Block 2: hidden_dim -> hidden_dim (with skip connection)
  ...
...
Diversity: ⭐⭐⭐⭐ (Skip connections enable deeper networks)
...
class CFG:
    VERSION_NAME = "v60"
    EXP_ID = "S6E3_V60_TabularResNet_V16Features"
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"Architecture: Tabular ResNet (Skip Connections)")
    print(f"Hidden_dim: {RESNET_PARAMS['hidden_dim']}, Num_blocks: {RESNET_PARAMS['num_blocks']}, Dropout: {RESNET_PARAMS['dropout']}")
    ...
    # [3/5] Training
    print(f"\n[3/5] Training Tabular ResNet ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, y_all)):
        ...
        # Convert to tensors
        X_tr_t = torch.tensor(X_tr_arr, dtype=torch.float32).to(DEVICE)
        y_tr_t = torch.tensor(y_tr, dtype=torch.float32).to(DEVICE)
        X_val_t = torch.tensor(X_val_arr, dtype=torch.float32).to(DEVICE)
        y_val_t = torch.tensor(y_val, dtype=torch.float32).to(DEVICE)
        X_te_t = torch.tensor(X_te_arr, dtype=torch.float32).to(DEVICE)
        ...
        # Initialize model
        model = TabularResNet(input_dim=len(ALL_FEATURES), **RESNET_PARAMS).to(DEVICE)
        optimizer = torch.optim.Adam(model.parameters(), lr=TRAIN_PARAMS['lr'], weight_decay=1e-4)
    ...
    print(f"\n{'='*80}")
    print(f"V60 RESULTS — Tabular ResNet + V16 Features")
    print(f"{'='*80}")
