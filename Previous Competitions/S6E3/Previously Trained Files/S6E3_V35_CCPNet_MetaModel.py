"""
S6E3 V35 - CCP-Net Style Meta-Learner (Stacking Level 2)
================================================================================
Strategy: Use CCP-Net architecture (Attention + BiLSTM + CNN) as meta-learner
          to combine OOF predictions from multiple base models
...
Key Insight:
  - CCP-Net failed as base model on raw features (wrong architecture for tabular)
  - BUT can work as meta-learner on prediction probabilities (sequential-like)
  - Treats OOF predictions as a "sequence" to learn non-linear combinations
...
class CCPNetMeta(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=64, num_heads=4, num_lstm_layers=1, 
        ...
        self.cnn = CNNBlock(hidden_dim, cnn_channels, kernel_sizes=[1, 2], dropout=dropout)
        
        # Global pooling + Classification head
        lstm_out_dim = hidden_dim  # BiLSTM outputs hidden_dim (bidirectional: hidden//2 * 2)
        cnn_out_dim = cnn_channels * 2  # 2 kernel sizes
...
class CFG:
    VERSION = "v35"
    EXP_ID = "S6E3_V35_CCPNet_MetaModel"
    ...
    # CCP-Net Parameters (smaller for meta-learning)
    CCPNET_PARAMS = {
        'hidden_dim': 64,
        'num_heads': 4,
        'num_lstm_layers': 1,
...
def main():
    ...
    print("="*80)
    print("\nCCP-Net Style Meta-Learner Architecture:")
    print(f"  - Input: OOF predictions from {len(CFG.BASE_MODELS)} base models")
    print(f"  - Attention Heads: {CFG.CCPNET_PARAMS['num_heads']}")
    print(f"  - LSTM Layers: {CFG.CCPNET_PARAMS['num_lstm_layers']}")
    print(f"  - CNN Channels: {CFG.CCPNET_PARAMS['cnn_channels']}")
    print(f"  - Hidden Dim: {CFG.CCPNET_PARAMS['hidden_dim']}")
    ...
    print(f"\n{'='*80}")
    print(f"V35 CCP-NET META-MODEL RESULTS")
    print(f"{'='*80}")
    ...
    # Best single model
    best_base_auc = max(CFG.BASE_MODELS[name]['oof_auc'] for name in model_names)
    improvement = overall_auc - best_base_auc
    if improvement > 0:
        print(f"\n✓ CCP-Net Meta improves over best base model by +{improvement:.5f}")
    else:
        print(f"\n✗ CCP-Net Meta is worse than best base model by {improvement:.5f}")

    # Check correlation with simple average
    corr = np.corrcoef(oof_meta, simple_avg)[0, 1]
    print(f"\nCorrelation with Simple Average: {corr:.4f}")
