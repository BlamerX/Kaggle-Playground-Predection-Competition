"""
S6E3 V50 - XGBoost with Heavy Regularization
================================================================================
Strategy: XGBoost with VERY heavy regularization (forced simplicity)

Key Difference from V16:
  - V16: max_depth=5, min_child_weight=6, reg_alpha=3.5, reg_lambda=1.3
  - V50: max_depth=4, min_child_weight=50, reg_alpha=5.0, reg_lambda=10.0

Why This Adds Diversity:
  - Forces model to learn ONLY the most robust patterns
  - Shallow trees = simpler decision boundaries
  - High regularization = conservative predictions
  - Complementary to complex models in ensemble

Based on: V52 success (LB 0.91718), V16 baseline
...
class CFG:
    VERSION_NAME = "v50"
    EXP_ID = "S6E3_V50_XGBoost_HeavyRegularization"
...
# HEAVY REGULARIZATION Parameters
# Key changes from V16:
#   - max_depth: 5 → 4 (shallower)
#   - min_child_weight: 6 → 50 (require more samples per leaf)
#   - reg_alpha: 3.5 → 5.0 (stronger L1)
#   - reg_lambda: 1.3 → 10.0 (stronger L2)
#   - gamma: 0.79 → 1.0 (require larger loss reduction)
#   - subsample: 0.81 → 0.6 (less data per tree)
#   - colsample_bytree: 0.32 → 0.5 (more features, less variance)
#   - learning_rate: 0.0063 → 0.01 (faster learning for simpler model)
XGB_PARAMS = {
    'n_estimators': 50000,
    'learning_rate': 0.01,           # Faster (was 0.0063)
    'max_depth': 4,                  # Shallow (was 5)
    'subsample': 0.6,                # Lower (was 0.81)
    'colsample_bytree': 0.5,         # Higher (was 0.32)
    'min_child_weight': 50,          # HIGH (was 6)
    'reg_alpha': 5.0,                # HIGH L1 (was 3.5017)
    'reg_lambda': 10.0,              # HIGH L2 (was 1.2925)
    'gamma': 1.0,                    # Higher (was 0.790)
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("XGBoost with HEAVY Regularization")
    print("Key: Force model to learn only robust, simple patterns")
    
    print("\n[Regularization Comparison]")
    print(f"  {'Parameter':<20} {'V16':<15} {'V50':<15}")
    print(f"  {'-'*50}")
    print(f"  {'max_depth':<20} {'5':<15} {'4':<15}")
    print(f"  {'min_child_weight':<20} {'6':<15} {'50':<15}")
    print(f"  {'reg_alpha':<20} {'3.5':<15} {'5.0':<15}")
    print(f"  {'reg_lambda':<20} {'1.3':<15} {'10.0':<15}")
    print(f"  {'gamma':<20} {'0.79':<15} {'1.0':<15}")
    print(f"  {'subsample':<20} {'0.81':<15} {'0.6':<15}")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [5/5] Training with Heavy Regularization (10-Fold CV)
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n[5/5] Training XGBoost with Heavy Regularization ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # Prepare for XGBoost — convert categoricals
        for df in [X_tr, X_val, X_te]:
        ...
        print(f"   Fold {i+1} AUC : {fold_auc:.5f} | Best Iter: {model.best_iteration} | {(time.time()-t0)/60:.1f} min")
    ...
    mean_iter = np.mean(xgb_best_iterations)
    
    print(f"\n{'='*80}")
    print(f"V50 RESULTS — XGBoost Heavy Regularization")
    print(f"{'='*80}")
    ...
    print(f"Mean Best Iteration: {mean_iter:.0f}")
    
    print(f"\n[Comparison]")
    print(f"  V16 (standard reg): CV 0.91917, LB 0.91679")
    print(f"  V50 (heavy reg):    CV {overall_auc:.5f}")
    print(f"  Delta vs V16: {overall_auc - 0.91917:+.5f}")
    
    verdict = "🏆 IMPROVED" if overall_auc > 0.91917 + 0.00005 else "✅ MARGINAL" if overall_auc > 0.91917 else "⚠️ LOWER CV"
    print(f"Verdict: {verdict}")
    print(f"\n  Note: Lower CV is expected! Heavy reg = simpler model")
    print(f"        But it may HELP the ensemble by adding diversity!")
    print(f"        Simple patterns may correct overfit models")
