"""
S6E3 V29 - Two-Stage Ridge + CatBoost
================================================================================
...
Key Idea:
  ...
  Stage 2: Train CatBoost on V16 features + Ridge predictions as additional feature
  
  This follows the "simple → complex" staged learning philosophy:
  - Linear model captures easy linear relationships
  - CatBoost focuses on non-linear patterns + corrects Ridge's mistakes
...
class CFG:
    VERSION_NAME = "v29"
    EXP_ID = "S6E3_V29_Ridge_CatBoost"
...
if __name__ == "__main__":
    ...
    print("  Stage 2: CatBoost with Ridge predictions as feature (non-linear correction)")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # STAGE 2: CatBoost with Ridge predictions as feature
        # ═══════════════════════════════════════════════════════════════════════
        print(f"\n  [Stage 2] Training CatBoost with Ridge predictions...")
    ...
    print(f"  V29 CatBoost:   {cat_overall_auc:.5f}")
    print(f"  Delta:          {cat_overall_auc - 0.91900:+.5f}")
    ...
    ridge_oof_df.to_csv(f"oof_v29_ridge.csv", index=False)
    ridge_sub_df = pd.DataFrame({'id': test_ids, CFG.TARGET: ridge_pred})
    ridge_sub_df.to_csv(f"sub_v29_ridge.csv", index=False)
    ...
    print(f"  Saved: oof_v29_ridge.csv, sub_v29_ridge.csv")
