"""
S6E3 V37 - Two-Stage Ridge + XGB with V36 Hidden Features
================================================================================
Strategy: Ridge (Stage 1) → XGB (Stage 2 with Ridge predictions as feature)

Key Changes from V27:
  - Uses V36 feature set (V16 + Hidden Features)
  - Hidden features: fiber_m2m, risk_score, combined_risk, etc.
  - V27 achieved LB 0.91683 (tied best), V36 also LB 0.91683

Key Idea:
  Stage 1: Train Ridge on all features to capture linear patterns
  Stage 2: Train XGB on V36 features + Ridge predictions as additional feature
  
  This follows the "simple → complex" staged learning philosophy:
  - Linear model captures easy linear relationships
  - XGB focuses on non-linear patterns + corrects Ridge's mistakes
...
class CFG:
    VERSION_NAME = "V37"
    EXP_ID = "S6E3_V37_TwoStage_Ridge_XGB_V36Features"
    ...
    RIDGE_ALPHA = 10.0  # Standard regularization strength

# V12 Optuna-Optimized Parameters (proven best from V16/V36)
XGB_PARAMS = {
    'n_estimators': 50000,
...
if __name__ == "__main__":
    ...
    # Feature Setup (DIGIT_FEATURES already in NEW_NUMS)
    ...
    print("  Stage 2: XGB with Ridge predictions as feature (non-linear correction)")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        if i == 0:
            ...
            hidden_imp = imp[[c for c in HIDDEN_FEATURES if c in imp.index]].sort_values(ascending=False)
            for rank, (fname, fval) in enumerate(hidden_imp.items()):
            ...
            for rank, (fname, fval) in enumerate(top_imp.items()):
                marker = " ***" if fname in HIDDEN_FEATURES else ""
                print(f"    {rank+1:2d}. {fname:40s} {fval:.4f}{marker}")
    ...
    print(f"\n{'='*80}")
    print(f"V37 RESULTS — Two-Stage Ridge → XGB (V36 Features)")
    print(f"{'='*80}")
    print(f"\n[Stage 1] Ridge:")
    ...
    print(f"\n[Comparison]:")
    print(f"  V27 (V16 features): LB 0.91683")
    print(f"  V36 (V16 + Hidden): LB 0.91683")
    print(f"  V37 XGB:            {xgb_overall_auc:.5f}")
    print(f"  V36 Baseline:       0.91918")
    print(f"  Delta vs V36:       {xgb_overall_auc - 0.91918:+.5f}")
    print(f"  Ridge → XGB lift:   {xgb_overall_auc - ridge_overall_auc:+.5f}")
    
    # Verdict
    verdict = "🏆 IMPROVED" if xgb_overall_auc > 0.91918 + 0.00005 else "✅ MARGINAL" if xgb_overall_auc > 0.91918 + 0.00001 else "= SAME" if abs(xgb_overall_auc - 0.91918) < 0.00005 else "❌ WORSE"
    print(f"\nVerdict: {verdict}")
    ...
    ridge_oof_df.to_csv(f"oof_V37_ridge.csv", index=False)
    ridge_sub_df = pd.DataFrame({'id': test_ids, CFG.TARGET: ridge_pred})
    ridge_sub_df.to_csv(f"sub_V37_ridge.csv", index=False)
    
    # XGB files
    xgb_oof_df = pd.DataFrame({'id': train_ids, CFG.TARGET: xgb_oof})
    xgb_oof_df.to_csv(f"oof_{CFG.VERSION_NAME}.csv", index=False)
    xgb_sub_df = pd.DataFrame({'id': test_ids, CFG.TARGET: xgb_pred})
    xgb_sub_df.to_csv(f"sub_{CFG.VERSION_NAME}.csv", index=False)
    
    print(f"  Saved: oof_V37_ridge.csv, sub_V37_ridge.csv")
