"""
S6E3 V41 - Two-Stage Ridge + LightGBM with V36 Hidden Features (Multi-Seed)
================================================================================
Strategy: Ridge (Stage 1, trained once) → LightGBM (Stage 2, 5 seeds)

Key Design:
  - Train Ridge ONCE (linear model, deterministic - no need for multi-seed)
  - Train LightGBM 5 times with different seeds
  - Average LightGBM predictions across all seeds

Why Ridge only once?
  - Ridge is a linear model with no random initialization
  - Same alpha + same data = same predictions regardless of seed
  - Only LightGBM benefits from multi-seed (random subsampling, tree building)

Multi-Seed Benefits:
  - Reduces variance from LightGBM's random initialization
...
LightGBM Seeds: [42, 0, 2024, 1234, 314159] (5 seeds to fit 12hr limit)
...
Rules:
  ...
  - Ridge: 1 run (10 folds), LightGBM: 5 seeds × 10 folds = 50 models
  - Total estimated time: ~6.4 hours (fits within 12hr Kaggle limit)
"""
...
class CFG:
    VERSION_NAME = "V41"
    EXP_ID = "S6E3_V41_TwoStage_Ridge_LightGBM_MultiSeed"
    ...
    # Multi-seed configuration - LightGBM only (5 seeds to fit within 12hr limit)
    LGB_SEEDS = [42, 0, 2024, 1234, 314159]
    N_SEEDS = len(LGB_SEEDS)
    
    # Ridge Parameters
    RIDGE_ALPHA = 10.0

def get_lgb_params(seed):
    """Get LightGBM params with specific seed"""
    return {
        'n_estimators': 10000,
        'learning_rate': 0.00833,
        'max_depth': 7,
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("Two-Stage Strategy: Ridge → LightGBM (feature augmentation)")
    print("Feature Set: V36 (V16 + Hidden Features)")
    print(f"Ridge: ONCE with seed {CFG.RIDGE_SEED}")
    print(f"LightGBM: {CFG.N_SEEDS} seeds × {CFG.N_FOLDS} folds = {CFG.N_SEEDS * CFG.N_FOLDS} models")
    print(f"LightGBM Seeds: {CFG.LGB_SEEDS}")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [7/7] STAGE 2: Train LightGBM with 5 different seeds
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n[7/7] STAGE 2: Training LightGBM with {CFG.N_SEEDS} seeds...")
    print(f"  Using SAME Ridge predictions for all seeds")
    
    all_seed_lgb_oof = np.zeros((CFG.N_SEEDS, len(train)))
    all_seed_lgb_pred = np.zeros((CFG.N_SEEDS, len(test)))
    seed_results = []
    
    for seed_idx, seed in enumerate(CFG.LGB_SEEDS):
        print(f"\n{'='*60}")
        print(f"LightGBM SEED {seed_idx+1}/{CFG.N_SEEDS}: {seed}")
        print(f"{'='*60}")
        
        LGB_PARAMS = get_lgb_params(seed)
        skf_lgb = StratifiedKFold(n_splits=CFG.N_FOLDS, shuffle=True, random_state=seed)
        skf_inner_lgb = StratifiedKFold(n_splits=CFG.INNER_FOLDS, shuffle=True, random_state=seed)
        ...
        t0_seed = time.time()
        for i, (train_idx, val_idx) in enumerate(skf_lgb.split(train, y_all)):
            ...
            # TE encoding
            for j, (in_tr, in_va) in enumerate(skf_inner_lgb.split(X_tr, y_tr)):
                X_tr2 = X_tr.loc[in_tr, FEATURES + [CFG.TARGET]].copy()
                X_va2 = X_tr.loc[in_va, FEATURES].copy()
                for col in TE_COLUMNS:
                    tmp = X_tr2.groupby(col, observed=False)[CFG.TARGET].agg(STATS)
            ...
            lgb_pred += model.predict_proba(X_te[COLS_LGB])[:, 1] / CFG.N_FOLDS
            
            print(f"    LightGBM Fold {i+1} AUC: {fold_auc:.5f}")
            
            del X_tr, X_val, X_te, model
            gc.collect()
        
        all_seed_lgb_oof[seed_idx] = lgb_oof
        all_seed_lgb_pred[seed_idx] = lgb_pred
        
        seed_auc = roc_auc_score(y_all, lgb_oof)
        seed_time = (time.time() - t0_seed) / 60
        seed_results.append({'seed': seed, 'lgb_auc': seed_auc, 'time_min': seed_time})
        
        print(f"\n  Seed {seed} LightGBM AUC: {seed_auc:.5f} | Time: {seed_time:.1f} min")

    # ═══════════════════════════════════════════════════════════════════════════
    # RESULTS
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n{'='*80}")
    print(f"V41 RESULTS — Multi-Seed LightGBM (Ridge trained once)")
    print(f"{'='*80}")
    
    # Individual seed results
    print(f"\n[Individual LightGBM Seed Results]")
    print(f"{'Seed':<10} {'LightGBM AUC':<12} {'Time (min)':<10}")
    print("-" * 35)
    for r in seed_results:
        print(f"{r['seed']:<10} {r['lgb_auc']:.5f}      {r['time_min']:.1f}")
    
    mean_lgb_auc = np.mean([r['lgb_auc'] for r in seed_results])
    std_lgb_auc = np.std([r['lgb_auc'] for r in seed_results])
    
    print(f"\n[Seed Statistics]")
    print(f"  Mean LightGBM AUC: {mean_lgb_auc:.5f} ± {std_lgb_auc:.5f}")
    print(f"  Ridge AUC:         {ridge_overall_auc:.5f} (trained once)")
    
    # Ensemble
    final_lgb_oof = all_seed_lgb_oof.mean(axis=0)
    final_lgb_pred = all_seed_lgb_pred.mean(axis=0)
    ensemble_lgb_auc = roc_auc_score(y_all, final_lgb_oof)
    
    print(f"\n[Ensemble Results]")
    print(f"  LightGBM Ensemble AUC:   {ensemble_lgb_auc:.5f}")
    print(f"  Lift from averaging:     {ensemble_lgb_auc - mean_lgb_auc:+.5f}")
    
    print(f"\n[Comparison]")
    print(f"  V28c (single seed): LB ~0.91648, CV ~0.91908")
    print(f"  V41 LightGBM Ensemble:  {ensemble_lgb_auc:.5f}")
    
    verdict = "IMPROVED" if ensemble_lgb_auc > 0.91908 + 0.00005 else "MARGINAL" if ensemble_lgb_auc > 0.91908 else "WORSE"
    print(f"\nVerdict: {verdict}")
    
    # Save
    print(f"\n Saving V41 results...")
    
    pd.DataFrame({'id': train_ids, CFG.TARGET: final_lgb_oof}).to_csv(f"oof_{CFG.VERSION_NAME}.csv", index=False)
    pd.DataFrame({'id': test_ids, CFG.TARGET: final_lgb_pred}).to_csv(f"sub_{CFG.VERSION_NAME}.csv", index=False)
    ...
    print(f"  LightGBM: ~{total_time_min - 5:.0f} min (5 seeds)")
    print("="*80)
