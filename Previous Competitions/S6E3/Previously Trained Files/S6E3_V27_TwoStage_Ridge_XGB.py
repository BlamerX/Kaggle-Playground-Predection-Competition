...
Key Idea:
  ...
  Stage 2: Train XGB on V16 features + Ridge predictions as additional feature
...
Rules:
  ...
  - Use exact V16 feature engineering (Optuna-optimized)
"""
...
class CFG:
    VERSION_NAME = "v27"
    EXP_ID = "S6E3_V27_TwoStage_Ridge_XGB"
...
# V12 Optuna-Optimized Parameters (proven best from V16)
XGB_PARAMS = {
    'n_estimators': 50000,
...
if __name__ == "__main__":
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [2/6] Feature Engineering — Core (V7 -> V14 baseline) — EXACT V16 COPY
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [3/6] Digit Features (V16) — EXACT COPY
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [4/6] Bi-gram / Tri-gram Composite Categoricals (V14 Baseline)
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # Ridge needs: standardized numerical + one-hot encoded categoricals
    ...
    print(f"\n{'='*80}")
    print(f"V27 RESULTS — Two-Stage Ridge → XGB")
    print(f"{'='*80}")
    print(f"\n[Stage 1] Ridge:")
    ...
    print(f"  V27 XGB:        {xgb_overall_auc:.5f}")
    print(f"  Delta:          {xgb_overall_auc - 0.91925:+.5f}")
    print(f"  Ridge → XGB lift: {xgb_overall_auc - ridge_overall_auc:+.5f}")
    
    # Verdict
    verdict = "🏆 IMPROVED" if xgb_overall_auc > 0.91925 + 0.00005 else "✅ MARGINAL" if xgb_overall_auc > 0.91925 + 0.00001 else "= SAME" if abs(xgb_overall_auc - 0.91925) < 0.00005 else "❌ WORSE"
    print(f"\nVerdict: {verdict}")
    ...
    ridge_oof_df.to_csv(f"oof_v27_ridge.csv", index=False)
    ridge_sub_df = pd.DataFrame({'id': test_ids, CFG.TARGET: ridge_pred})
    ridge_sub_df.to_csv(f"sub_v27_ridge.csv", index=False)
    ...
    print(f"  Saved: oof_v27_ridge.csv, sub_v27_ridge.csv")
