"""
S6E3 V70 - LightGBM + Difficulty Weighting
================================================================================
Strategy: Weight hard samples more during training (two-stage training)

Key Idea:
  1. Stage 1: Train initial LightGBM model
  2. Compute difficulty: |prediction - actual| (higher = harder sample)
  3. Stage 2: Retrain with sample weights based on difficulty
  4. Hard samples get more weight, easy samples get less

Reference: 
  - V20 LightGBM (OOF: 0.91908, LB: 0.91661)
  - Paper: "Focal Loss for Dense Object Detection" (hard example mining)

Implementation:
  - Stage 1: Train model normally
  - Compute difficulty for each training sample
  - Stage 2: Retrain with sample_weight = 0.5 + difficulty

Diversity: ⭐⭐⭐⭐ (Different optimization focus)
...
class CFG:
    VERSION_NAME = "v70"
    EXP_ID = "S6E3_V70_LightGBM_DifficultyWeighting"
    ...
    # Difficulty Weighting Parameters
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"Strategy: LightGBM with Two-Stage Difficulty Weighting")
    print(f"Stage 1: Train initial model ({CFG.STAGE1_ITERATIONS} iterations)")
    print(f"Stage 2: Retrain with difficulty-based weights ({CFG.STAGE2_ITERATIONS} iterations)")
    print(f"Weight range: [{CFG.MIN_WEIGHT}, {CFG.MAX_WEIGHT}]")
    ...
    # [4/5] Two-Stage Training with Difficulty Weighting
    print(f"\n[4/5] Training LightGBM with Difficulty Weighting ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf.split(train, y_all)):
        ...
        # ═══════════════════════════════════════════════════════════════════════
        print(f"  Stage 1: Training initial model...")
        lgb_train_s1 = lgb.Dataset(X_tr, label=y_tr)
        lgb_valid_s1 = lgb.Dataset(X_val, label=y_val, reference=lgb_train_s1)
        
        model_s1 = lgb.train(
            LGB_PARAMS,
            lgb_train_s1,
            num_boost_round=CFG.STAGE1_ITERATIONS,
            valid_sets=[lgb_valid_s1],
            callbacks=[lgb.log_evaluation(period=500)],
        )
        
        # Get predictions and compute difficulty
        train_preds_s1 = model_s1.predict(X_tr)
        val_preds_s1 = model_s1.predict(X_val)
        
        # Difficulty = absolute error (higher = harder)
        difficulty = np.abs(train_preds_s1 - y_tr)
        
        # Normalize difficulty to [0, 1]
        difficulty_norm = (difficulty - difficulty.min()) / (difficulty.max() - difficulty.min() + 1e-8)
        
        # Create sample weights: range [MIN_WEIGHT, MAX_WEIGHT]
        sample_weights = CFG.MIN_WEIGHT + difficulty_norm * (CFG.MAX_WEIGHT - CFG.MIN_WEIGHT)
        
        # Statistics
        print(f"  Difficulty stats: min={difficulty.min():.4f}, max={difficulty.max():.4f}, mean={difficulty.mean():.4f}")
        print(f"  Sample weights: min={sample_weights.min():.2f}, max={sample_weights.max():.2f}, mean={sample_weights.mean():.2f}")
        
        stage1_auc = roc_auc_score(y_val, val_preds_s1)
        print(f"  Stage 1 AUC: {stage1_auc:.5f}")
        
        # ═══════════════════════════════════════════════════════════════════════
        # STAGE 2: Retrain with difficulty-based weights
        # ═══════════════════════════════════════════════════════════════════════
        print(f"  Stage 2: Retraining with difficulty weights...")
        lgb_train_s2 = lgb.Dataset(X_tr, label=y_tr, weight=sample_weights)
        lgb_valid_s2 = lgb.Dataset(X_val, label=y_val, reference=lgb_train_s2)
        
        model_s2 = lgb.train(
            LGB_PARAMS,
            lgb_train_s2,
            num_boost_round=CFG.STAGE2_ITERATIONS,
            valid_sets=[lgb_valid_s2],
            callbacks=[lgb.log_evaluation(period=500)],
        )
        
        val_preds_s2 = model_s2.predict(X_val)
        test_preds_s2 = model_s2.predict(X_te)
        
        stage2_auc = roc_auc_score(y_val, val_preds_s2)
        
        oof[val_idx] = val_preds_s2
        pred += test_preds_s2 / CFG.N_FOLDS
        fold_scores.append(stage2_auc)
        
        delta = stage2_auc - stage1_auc
        print(f"  Stage 2 AUC: {stage2_auc:.5f} | Δ: {delta:+.5f}")
        print(f"  Time: {(time.time()-t0)/60:.1f} min")
        
        del model_s1, model_s2, lgb_train_s1, lgb_train_s2, lgb_valid_s1, lgb_valid_s2
        gc.collect()
    ...
    print(f"\n{'='*80}")
    print(f"V70 - LightGBM + Difficulty Weighting")
    print(f"{'='*80}")
