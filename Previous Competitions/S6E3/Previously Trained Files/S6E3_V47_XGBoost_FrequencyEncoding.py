"""
S6E3 V47 - XGBoost with Frequency Encoding
================================================================================
Strategy: XGBoost using FREQUENCY encoding for categoricals (NOT label encoding)

Key Difference from V16:
  - V16: Label encoding → "Which category is this?"
  - V47: Frequency encoding → "How common is this category?"

Why This Adds Diversity:
  - Frequency encoding captures popularity/rarity
  - Different numerical representation = different splits
  - Trees split on "is this value common?" not "is this value X?"
  - Orthogonal to all existing label-encoded models

Based on: V52 success (LB 0.91718), V16 parameters (Optuna optimized)
...
class CFG:
    VERSION_NAME = "v47"
    EXP_ID = "S6E3_V47_XGBoost_FrequencyEncoding"
...
# V16 Optuna-Optimized Parameters (proven best)
XGB_PARAMS = {
    'n_estimators': 50000,
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("XGBoost with FREQUENCY Encoding")
    print("Key: Replace categorical values with their frequency (popularity)")
    ...
    # 1. Frequency Encoding for NUMERICALS (same as V16)
    for col in NUMS:
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [4/5] KEY DIFFERENCE: Frequency Encoding for Categoricals
    # ═══════════════════════════════════════════════════════════════════════════
    print("\n[4/5] Creating Frequency Encoded Features (KEY DIFFERENCE)...")
    
    # Frequency encoding for ALL categoricals
    FREQ_CAT_COLS = []
    for col in CATS:
        freq_name = f'FREQ_CAT_{col}'
        FREQ_CAT_COLS.append(freq_name)
        
        # Compute frequency from training data only (no leakage)
        freq = train[col].value_counts(normalize=True)
        
        for df in [train, test]:
            df[freq_name] = df[col].map(freq).fillna(0).astype('float32')
    
    print(f"  Created {len(FREQ_CAT_COLS)} frequency encoded categorical features")
    print(f"  Example: Contract frequency encoding")
    print(f"    {train['Contract'].value_counts(normalize=True).head()}")
    
    # N-gram with frequency encoding as well
    BIGRAM_COLS = []
    TRIGRAM_COLS = []
    ...
    # Frequency encoding for N-grams too
    FREQ_NGRAM_COLS = []
    for col in NGRAM_COLS:
        freq_name = f'FREQ_NG_{col}'
        FREQ_NGRAM_COLS.append(freq_name)
    ...
    print(f"  Created {len(FREQ_NGRAM_COLS)} frequency encoded N-gram features")
    
    # ═══════════════════════════════════════════════════════════════════════════
    # Feature Setup - NO label encoding, use frequency encoding
    # ═══════════════════════════════════════════════════════════════════════════
    FEATURES = NUMS + FREQ_CAT_COLS + NEW_NUMS + FREQ_NGRAM_COLS
    
    print(f"\n  Total features: {len(FEATURES)}")
    print(f"  Frequency encoded categoricals: {len(FREQ_CAT_COLS)}")
    print(f"  Frequency encoded N-grams: {len(FREQ_NGRAM_COLS)}")
    
    # ═══════════════════════════════════════════════════════════════════════════
    # [5/5] Training (10-Fold CV)
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n[5/5] Training XGBoost with Frequency Encoding ({CFG.N_FOLDS}-Fold CV)...")
    ...
    print(f"\n{'='*80}")
    print(f"V47 RESULTS — XGBoost Frequency Encoding")
    print(f"{'='*80}")
    ...
    print(f"\n[Comparison]")
    print(f"  V16 (label encoding):  CV 0.91917, LB 0.91679")
    print(f"  V47 (freq encoding):   CV {overall_auc:.5f}")
    print(f"  Delta vs V16: {overall_auc - 0.91917:+.5f}")
    
    verdict = "🏆 IMPROVED" if overall_auc > 0.91917 + 0.00005 else "✅ MARGINAL" if overall_auc > 0.91917 else "⚠️ SAME/WORSE"
    print(f"Verdict: {verdict}")
    print(f"\n  Note: Even if CV is lower, diversity helps ensemble!")
