"""
S6E3 V40 - Two-Stage Ridge + CatBoost with V36 Hidden Features (Multi-Seed)
================================================================================
Strategy: Ridge (Stage 1, trained once) → CatBoost (Stage 2, 10 seeds)

Key Design:
  - Train Ridge ONCE (linear model, deterministic - no need for multi-seed)
  - Train CatBoost 10 times with different seeds
  - Average CatBoost predictions across all seeds

Why Ridge only once?
  - Ridge is a linear model with no random initialization
  - Same alpha + same data = same predictions regardless of seed
  - Only CatBoost benefits from multi-seed (random bootstrap, tree building)

Multi-Seed Benefits:
  - Reduces variance from CatBoost's random initialization
  - Different CV splits produce diverse tree structures
  - Averaging improves generalization to LB

Feature Set: V16 + Hidden Features (from V36)
  - fiber_m2m, risk_score, combined_risk, contract_risk
  - internet_risk, is_brand_new, stream_no_prot, payment_risk

CatBoost Seeds: [42, 0, 1, 2, 3, 2024, 2025, 1234, 12345, 314159]
...
Rules:
  ...
  - Ridge: 1 run (10 folds), CatBoost: 10 seeds × 10 folds = 100 models
"""
...
class CFG:
    VERSION_NAME = "V40"
    EXP_ID = "S6E3_V40_TwoStage_Ridge_CatBoost_MultiSeed"
    ...
    # Multi-seed configuration - CatBoost only
    CAT_SEEDS = [42, 0, 1, 2, 3, 2024, 2025, 1234, 12345, 314159]
    N_SEEDS = len(CAT_SEEDS)
    
    # Ridge Parameters
    RIDGE_ALPHA = 10.0

def get_cat_params(seed):
    """Get CatBoost params with specific seed"""
    return {
        'iterations': 20000,
        'learning_rate': 0.00984,
        'depth': 7,
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("Two-Stage Strategy: Ridge → CatBoost (feature augmentation)")
    print("Feature Set: V36 (V16 + Hidden Features)")
    print(f"Ridge: ONCE with seed {CFG.RIDGE_SEED}")
    print(f"CatBoost: {CFG.N_SEEDS} seeds × {CFG.N_FOLDS} folds = {CFG.N_SEEDS * CFG.N_FOLDS} models")
    print(f"CatBoost Seeds: {CFG.CAT_SEEDS}")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [7/7] STAGE 2: Train CatBoost with 10 different seeds
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n[7/7] STAGE 2: Training CatBoost with {CFG.N_SEEDS} seeds...")
    print(f"  Using SAME Ridge predictions for all seeds")
    
    all_seed_cat_oof = np.zeros((CFG.N_SEEDS, len(train)))
    all_seed_cat_pred = np.zeros((CFG.N_SEEDS, len(test)))
    seed_results = []
    
    for seed_idx, seed in enumerate(CFG.CAT_SEEDS):
        print(f"\n{'='*60}")
        print(f"CatBoost SEED {seed_idx+1}/{CFG.N_SEEDS}: {seed}")
        print(f"{'='*60}")
        
        CAT_PARAMS = get_cat_params(seed)
        skf_cat = StratifiedKFold(n_splits=CFG.N_FOLDS, shuffle=True, random_state=seed)
        skf_inner_cat = StratifiedKFold(n_splits=CFG.INNER_FOLDS, shuffle=True, random_state=seed)
        ...
        t0_seed = time.time()
        for i, (train_idx, val_idx) in enumerate(skf_cat.split(train, y_all)):
            ...
            # TE encoding
            for j, (in_tr, in_va) in enumerate(skf_inner_cat.split(X_tr, y_tr)):
                X_tr2 = X_tr.loc[in_tr, FEATURES + [CFG.TARGET]].copy()
                X_va2 = X_tr.loc[in_va, FEATURES].copy()
                for col in TE_COLUMNS:
                    tmp = X_tr2.groupby(col, observed=False)[CFG.TARGET].agg(STATS)
            ...
            cat_pred += model.predict_proba(X_te[COLS_CAT])[:, 1] / CFG.N_FOLDS
            
            print(f"    CatBoost Fold {i+1} AUC: {fold_auc:.5f}")
            
            del X_tr, X_val, X_te, model, train_pool, val_pool
            gc.collect()
        
        all_seed_cat_oof[seed_idx] = cat_oof
        all_seed_cat_pred[seed_idx] = cat_pred
        
        seed_auc = roc_auc_score(y_all, cat_oof)
        seed_time = (time.time() - t0_seed) / 60
        seed_results.append({'seed': seed, 'cat_auc': seed_auc, 'time_min': seed_time})
        
        print(f"\n  Seed {seed} CatBoost AUC: {seed_auc:.5f} | Time: {seed_time:.1f} min")

    # ═══════════════════════════════════════════════════════════════════════════
    # RESULTS
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n{'='*80}")
    print(f"V40 RESULTS — Multi-Seed CatBoost (Ridge trained once)")
    print(f"{'='*80}")
    
    # Individual seed results
    print(f"\n[Individual CatBoost Seed Results]")
    print(f"{'Seed':<10} {'CatBoost AUC':<12} {'Time (min)':<10}")
    print("-" * 35)
    for r in seed_results:
        print(f"{r['seed']:<10} {r['cat_auc']:.5f}      {r['time_min']:.1f}")
    
    mean_cat_auc = np.mean([r['cat_auc'] for r in seed_results])
    std_cat_auc = np.std([r['cat_auc'] for r in seed_results])
    
    print(f"\n[Seed Statistics]")
    print(f"  Mean CatBoost AUC: {mean_cat_auc:.5f} ± {std_cat_auc:.5f}")
    print(f"  Ridge AUC:         {ridge_overall_auc:.5f} (trained once)")
    
    # Ensemble
    final_cat_oof = all_seed_cat_oof.mean(axis=0)
    final_cat_pred = all_seed_cat_pred.mean(axis=0)
    ensemble_cat_auc = roc_auc_score(y_all, final_cat_oof)
    
    print(f"\n[Ensemble Results]")
    print(f"  CatBoost Ensemble AUC:    {ensemble_cat_auc:.5f}")
    print(f"  Lift from averaging:      {ensemble_cat_auc - mean_cat_auc:+.5f}")
    
    print(f"\n[Comparison]")
    print(f"  V29b (single seed): LB ~0.91646, CV ~0.91900")
    print(f"  V40 CatBoost Ensemble:  {ensemble_cat_auc:.5f}")
    
    verdict = "IMPROVED" if ensemble_cat_auc > 0.91900 + 0.00005 else "MARGINAL" if ensemble_cat_auc > 0.91900 else "WORSE"
    print(f"\nVerdict: {verdict}")
    
    # Save
    print(f"\n Saving V40 results...")
    
    pd.DataFrame({'id': train_ids, CFG.TARGET: final_cat_oof}).to_csv(f"oof_{CFG.VERSION_NAME}.csv", index=False)
    pd.DataFrame({'id': test_ids, CFG.TARGET: final_cat_pred}).to_csv(f"sub_{CFG.VERSION_NAME}.csv", index=False)
    ...
    print(f"  CatBoost: ~{total_time_min - 5:.0f} min (10 seeds)")
    print("="*80)
