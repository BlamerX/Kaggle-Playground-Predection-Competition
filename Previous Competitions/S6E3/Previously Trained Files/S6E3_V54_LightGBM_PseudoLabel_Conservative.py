"""
S6E3 V54 - LightGBM + Pseudo-Labeling (Conservative)
================================================================================
Strategy: V20 Reference + Conservative Pseudo-Labeling

Key Idea:
  Use V20's test predictions as pseudo-labels for training.
...
class CFG:
    VERSION_NAME = "v54"
    EXP_ID = "S6E3_V54_LightGBM_PseudoLabel_Conservative"
...
# LightGBM Parameters (Same as V20 - Optuna optimized)
LGB_PARAMS = {
    'n_estimators': 20000,
...
if __name__ == "__main__":
    ...
    # [3/5] Load V20 Predictions for Pseudo-Labels
    ...
    print(f"\n[4/5] Training LightGBM with Pseudo-Labels ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # Record Results
        lgb_oof[val_idx_original] = model.predict_proba(X_val)[:, 1]
        fold_auc = roc_auc_score(y_val, lgb_oof[val_idx_original])
    ...
    print(f"\n{'='*80}")
    print(f"V54 RESULTS — LightGBM + Pseudo-Labeling (Conservative)")
    print(f"{'='*80}")
    print(f"Overall CV AUC:  {overall_auc:.5f} (Mean: {mean_score:.5f} +/- {std_score:.5f})")
    print(f"V20 Reference:  0.91908 (OOF)")
