"""
S6E3 V67 - XGBoost + Cost-Sensitive Learning
================================================================================
Strategy: Different misclassification costs for churn vs non-churn

Key Idea:
  - Emphasize churn detection (positive class) by adjusting class weights
  - scale_pos_weight = (num_negative / num_positive) * 2.0
  - This increases the cost of missing a churner (false negative)
  
Reference:
  - V16b (OOF: 0.91925, LB: 0.91680) - XGBoost baseline

Why Cost-Sensitive?
  - In churn prediction, missing a churner is typically more costly
  - Standard models optimize for overall accuracy
  - Cost-sensitive learning shifts focus to the minority class
...
class CFG:
    VERSION_NAME = "v67"
    EXP_ID = "S6E3_V67_XGBoost_CostSensitive"
    ...
    SCALE_POS_WEIGHT_MULTIPLIER = 2.0  # Multiply standard weight by this factor

# XGBoost Parameters (from V16b, with cost-sensitive modification)
XGB_PARAMS = {
    'n_estimators': 50000,
    ...
    'device': 'cuda',
    'verbosity': 0,
    # scale_pos_weight will be set dynamically based on class distribution
}
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("Cost-Sensitive Learning Strategy:")
    print("  - Increase weight for positive class (churn)")
    print(f"  - scale_pos_weight multiplier: {CFG.SCALE_POS_WEIGHT_MULTIPLIER}x")
    print("  - This increases the cost of false negatives")
    ...
    # Calculate class distribution and scale_pos_weight
    num_positive = train[CFG.TARGET].sum()
    num_negative = len(train) - num_positive
    standard_scale_pos_weight = num_negative / num_positive
    enhanced_scale_pos_weight = standard_scale_pos_weight * CFG.SCALE_POS_WEIGHT_MULTIPLIER
    
    print(f"\n[Class Distribution]:")
    print(f"  Positive (Churn): {num_positive:,} ({100*num_positive/len(train):.2f}%)")
    print(f"  Negative (No Churn): {num_negative:,} ({100*num_negative/len(train):.2f}%)")
    print(f"  Standard scale_pos_weight: {standard_scale_pos_weight:.4f}")
    print(f"  Enhanced scale_pos_weight: {enhanced_scale_pos_weight:.4f} ({CFG.SCALE_POS_WEIGHT_MULTIPLIER}x)")
    
    # Update XGB_PARAMS with cost-sensitive weight
    XGB_PARAMS['scale_pos_weight'] = enhanced_scale_pos_weight
    ...
    # [3/5] Training with Cost-Sensitive Learning
    print(f"\n[3/5] Training XGBoost with Cost-Sensitive Learning ({CFG.N_FOLDS}-Fold CV)...")
    ...
    print(f"\n{'='*80}")
    print(f"V67 RESULTS — XGBoost + Cost-Sensitive Learning")
    print(f"{'='*80}")
    print(f"\n[Cost-Sensitive Settings]:")
    print(f"  scale_pos_weight: {enhanced_scale_pos_weight:.4f} ({CFG.SCALE_POS_WEIGHT_MULTIPLIER}x standard)")
    print(f"  This increases the cost of false negatives (missing churners)")
    print(f"\n[Performance]:")
    ...
    verdict = "IMPROVED" if overall_auc > 0.91925 else "MARGINAL" if overall_auc > 0.91920 else "SAME"
