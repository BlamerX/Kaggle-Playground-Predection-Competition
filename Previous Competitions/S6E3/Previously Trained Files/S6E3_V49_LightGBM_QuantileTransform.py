"""
S6E3 V49 - LightGBM with Quantile Transformation
================================================================================
Strategy: LightGBM with Quantile-transformed numerical features

Key Difference from V20:
  - V20: Raw numerical features (skewed distributions)
  - V49: Quantile-transformed to Gaussian distribution

Why This Adds Diversity:
  - Different feature distribution = different split points
  - Forces trees to split at Gaussian-optimized thresholds
  - Outliers handled differently
  - Same algorithm, fundamentally different feature view

Based on: V52 success (LB 0.91718), V20 parameters (Optuna optimized)
...
class CFG:
    VERSION_NAME = "v49"
    EXP_ID = "S6E3_V49_LightGBM_QuantileTransform"
...
# V20 Optuna-Optimized Parameters (proven best)
LGB_PARAMS = {
    'n_estimators': 20000,
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("LightGBM with Quantile Transformation")
    print("Key: Transform numericals to Gaussian distribution → different splits")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # [5/5] Training with Quantile Transform (10-Fold CV)
    # ═══════════════════════════════════════════════════════════════════════════
    print(f"\n[5/5] Training LightGBM with Quantile Transform ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # =========================================
        # KEY DIFFERENCE: Quantile Transform numericals
        # =========================================
        # Identify numerical columns (all columns after removing categoricals)
        QT_COLS = NUMS + NEW_NUMS  # These are the numerical features
        
        # Fit QT on training fold, transform all
        qt = QuantileTransformer(output_distribution='normal', random_state=CFG.RANDOM_SEED)
        
        # Get indices of numerical columns in current feature set
        qt_col_indices = [COLS_LGB.get_loc(c) for c in QT_COLS if c in COLS_LGB]
        
        if i == 0:
            print(f"  Applying Quantile Transform to {len(qt_col_indices)} numerical features")
            print(f"  Example: TotalCharges before QT: min={X_tr['TotalCharges'].min():.2f}, max={X_tr['TotalCharges'].max():.2f}")
        
        # Transform numerical columns
        X_tr_qt = X_tr.values.copy()
        X_val_qt = X_val.values.copy()
        X_te_qt = X_te.values.copy()
        
        # Fit on training data, transform all
        X_tr_qt[:, qt_col_indices] = qt.fit_transform(X_tr.values[:, qt_col_indices])
        X_val_qt[:, qt_col_indices] = qt.transform(X_val.values[:, qt_col_indices])
        X_te_qt[:, qt_col_indices] = qt.transform(X_te.values[:, qt_col_indices])
        
        # Convert back to DataFrame
        X_tr = pd.DataFrame(X_tr_qt, columns=COLS_LGB)
        X_val = pd.DataFrame(X_val_qt, columns=COLS_LGB)
        X_te = pd.DataFrame(X_te_qt, columns=COLS_LGB)
        
        if i == 0:
            print(f"  Example: TotalCharges after QT: min={X_tr['TotalCharges'].min():.2f}, max={X_tr['TotalCharges'].max():.2f}")
        ...
        del X_tr, X_val, X_te, y_tr, y_val, model, qt
        gc.collect()
    ...
    print(f"\n{'='*80}")
    print(f"V49 RESULTS — LightGBM Quantile Transform")
    print(f"{'='*80}")
    ...
    print(f"\n[Comparison]")
    print(f"  V20 (raw features): CV 0.91908, LB 0.91661")
    print(f"  V49 (QT features):  CV {overall_auc:.5f}")
    print(f"  Delta vs V20: {overall_auc - 0.91908:+.5f}")
    
    verdict = "🏆 IMPROVED" if overall_auc > 0.91908 + 0.00005 else "✅ MARGINAL" if overall_auc > 0.91908 else "⚠️ SAME/WORSE"
    print(f"Verdict: {verdict}")
    print(f"\n  Note: QT changes split points → adds diversity!")
