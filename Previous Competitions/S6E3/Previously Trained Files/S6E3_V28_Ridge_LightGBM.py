"""
S6E3 V28 - Two-Stage Ridge + LightGBM
================================================================================
...
Key Idea:
  ...
  Stage 2: Train LightGBM on V16 features + Ridge predictions as additional feature
  
  This follows the "simple → complex" staged learning philosophy:
  - Linear model captures easy linear relationships
  - LightGBM focuses on non-linear patterns + corrects Ridge's mistakes

Parameters from V20 Optuna:
...
Rules:
  ...
  - Use exact V16 feature engineering
"""
...
class CFG:
    VERSION_NAME = "v28"
    EXP_ID = "S6E3_V28_Ridge_LightGBM"
...
# LightGBM Optuna-Optimized Parameters from V20
LGB_PARAMS = {
    'n_estimators': 20000,
    'learning_rate': 0.00833,
    'max_depth': 7,
...
if __name__ == "__main__":
    ...
    print("  Stage 2: LightGBM with Ridge predictions as feature (non-linear correction)")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # STAGE 2: LightGBM with Ridge predictions as feature
        # ═══════════════════════════════════════════════════════════════════════
        print(f"\n  [Stage 2] Training LightGBM with Ridge predictions...")
    ...
    print(f"\n{'='*80}")
    print(f"V28 RESULTS — Two-Stage Ridge → LightGBM")
    print(f"{'='*80}")
    print(f"\n[Stage 1] Ridge:")
    ...
    print(f"  V28 LightGBM:   {lgb_overall_auc:.5f}")
    print(f"  Delta:          {lgb_overall_auc - 0.91908:+.5f}")
    ...
    ridge_oof_df.to_csv(f"oof_v28_ridge.csv", index=False)
    ridge_sub_df = pd.DataFrame({'id': test_ids, CFG.TARGET: ridge_pred})
    ridge_sub_df.to_csv(f"sub_v28_ridge.csv", index=False)
    ...
    print(f"  Saved: oof_v28_ridge.csv, sub_v28_ridge.csv")
