"""
S6E3 V57 - XGBoost + Pseudo-Labeling (Aggressive)
================================================================================
Strategy: V16b Reference + Aggressive Pseudo-Labeling

Key Idea:
  Use V16b's test predictions as pseudo-labels with lower threshold.
  More pseudo-labeled samples = more training data, but more noise risk.
  
Teacher:
  - V52: LB 0.91718 (Best Hill Climbing Ensemble)
  - Using best predictions for highest quality pseudo-labels
  
Pseudo-Labeling Parameters (Aggressive):
  - Threshold: prob > 0.95 or < 0.05 (more samples)
  - Sample Weight: 0.3 (lower weight to handle noise)
...
class CFG:
    VERSION_NAME = "v57"
    EXP_ID = "S6E3_V57_XGBoost_PseudoLabel_Aggressive"
    ...
    # Pseudo-Labeling Parameters (Aggressive)
    PL_THRESHOLD_HIGH = 0.95   # Lower threshold = more samples
    PL_THRESHOLD_LOW = 0.05
    PL_WEIGHT = 0.3            # Lower weight to handle potential noise

# XGBoost Parameters (Same as V16b)
XGB_PARAMS = {
    'n_estimators': 50000,
...
if __name__ == "__main__":
    ...
    print(f"Pseudo-Label Thresholds: >={CFG.PL_THRESHOLD_HIGH} or <={CFG.PL_THRESHOLD_LOW} (AGGRESSIVE)")
    print(f"Pseudo-Label Weight: {CFG.PL_WEIGHT} (lower for noise tolerance)")
    ...
    # [3/5] Load V16b Predictions for Pseudo-Labels
    ...
    print(f"  Total pseudo-labeled: {pseudo_mask.sum():,} (AGGRESSIVE - more samples)")
    ...
    print(f"\n[4/5] Training XGBoost with Pseudo-Labels ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        model.fit(X_tr, y_tr, sample_weight=w_tr_final, eval_set=[(X_val, y_val)], verbose=1000)
        
        # Record Results
        xgb_oof[val_idx_original] = model.predict_proba(X_val)[:, 1]
        fold_auc = roc_auc_score(y_val, xgb_oof[val_idx_original])
        ...
        del X_tr, X_val, X_te, y_tr, y_val, w_tr, model
        gc.collect()
    ...
    print(f"\n{'='*80}")
    print(f"V57 RESULTS — XGBoost + Pseudo-Labeling (Aggressive)")
    print(f"{'='*80}")
