"""
S6E3 V65 - XGBoost with V52 Teacher Pseudo-Labels
================================================================================
Strategy: Pseudo-labeling using V52 (best ensemble) as teacher

Key Idea:
  1. Use V52 predictions (LB 0.91718) as high-quality pseudo-labels
  2. Add only very high-confidence samples (threshold 0.98)
  3. Use low weight (0.3) for pseudo-labeled samples
  4. This creates diversity from V16b via different training data

Reference: 
  - V16b (OOF: 0.91925, LB: 0.91680)
  - V52 Teacher (LB 0.91718 - Best Hill Climber)
  - V53 (OOF 0.91920 - XGB + V52 pseudo-label)

Diversity: ⭐⭐⭐⭐ (Same architecture, different training data via V52 teacher)
...
class CFG:
    VERSION_NAME = "v65"
    EXP_ID = "S6E3_V65_XGBoost_V52Teacher"
    ...
    # V52 predictions (best ensemble as teacher)
    ...
    # Conservative Pseudo-Labeling Parameters
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"Strategy: XGBoost with V52 Teacher Pseudo-Labels")
    print(f"Teacher: V52 (LB 0.91718 - Best Hill Climber)")
    print(f"Threshold: >={CFG.PL_THRESHOLD_HIGH} or <={CFG.PL_THRESHOLD_LOW}")
    ...
    # [2/5] Feature Engineering - Do on train, test, orig FIRST
    ...
    # [3/5] Load V52 predictions and create augmented training data
    print(f"\n[3/5] Loading V52 teacher predictions and creating augmented data...")
    ...
    print(f"  High-conf churn (>{CFG.PL_THRESHOLD_HIGH}): {high_conf_churn.sum():,}")
    print(f"  High-conf no-churn (<{CFG.PL_THRESHOLD_LOW}): {high_conf_no_churn.sum():,}")
    print(f"  Total pseudo-labeled: {pseudo_mask.sum():,}")
    
    # Create augmented training data (after feature engineering)
    ...
    # [4/5] Training - Validate only on ORIGINAL train samples
    print(f"\n[4/5] Training XGBoost ({CFG.N_FOLDS}-Fold CV with Pseudo-Labels)...")
    print("  Note: Validation only on original train samples")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, y_all)):
        ...
        COLS_XGB = X_tr.columns.tolist()
        
        X_val = X_val[COLS_XGB]
        X_te = X_te[COLS_XGB]
        
        if i == 0:
            print(f"  Total features for XGB: {len(COLS_XGB)}")
        
        # Train
        model = xgb.XGBClassifier(**XGB_PARAMS)
        model.fit(X_tr, y_tr, sample_weight=w_tr, eval_set=[(X_val, y_val)], verbose=5000)
        ...
        pred += model.predict_proba(X_te)[:, 1] / CFG.N_FOLDS
    ...
    print(f"\n{'='*80}")
    print(f"V65 RESULTS — XGBoost + V52 Teacher Pseudo-Labels")
    print(f"{'='*80}")
    print(f"Overall CV AUC:  {overall_auc:.5f} (Mean: {mean_score:.5f} +/- {std_score:.5f})")
    print(f"V16b Reference:  0.91925 (OOF)")
    print(f"V53 Reference:   0.91920 (XGB + V52 pseudo-label)")
    print(f"Delta vs V16b:   {overall_auc - 0.91925:+.5f}")
    print(f"Per-fold: {' | '.join(f'{s:.5f}' for s in fold_scores)}")
    
    verdict = "IMPROVED" if overall_auc > 0.91925 else "MARGINAL" if overall_auc > 0.91915 else "SAME"
