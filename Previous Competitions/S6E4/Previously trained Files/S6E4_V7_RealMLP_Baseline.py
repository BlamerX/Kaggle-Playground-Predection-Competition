"""
S6E4 V7 - RealMLP Baseline (GPU - pytabkit)
================================================================================
Strategy: RealMLP with Digit Features + Target Encoding + Label Smoothing

Reference: https://www.kaggle.com/code/yekenot/ps-s6-e3-realmlp-pytabkit (LB 0.91667)
Reference implementation: S6E3_V72_RealMLP_Optimized.py

Key advantages of RealMLP over vanilla MLP (V7 old):
1. n_ens=32: built-in ensemble of 32 independent models (BagEnsemble)
2. Residual connections between layers (prevents gradient degradation)
3. Advanced preprocessing via tfms pipeline (median_center, robust_scale, smooth_clip, l2_normalize)
4. Label smoothing (ls_eps=0.02) for class imbalance
5. Efficient training with flat_annealing LR schedule
6. ReGLU-like activations (silu)
...
- RealMLP handles preprocessing internally via tfms

Why no sample_weight:
  pytabkit does not expose sample_weight in fit(). Class imbalance handled by:
  1. ls_eps=0.02 (label smoothing during training)
  2. Optuna weight optimization (post-training, 200 trials)

Bugs fixed from previous V7 (vanilla MLP):
1. Replaced vanilla MLP with RealMLP (much stronger architecture)
2. Added n_ens=32 built-in ensemble
3. Added label smoothing (ls_eps=0.02) for class imbalance
4. Added tfms preprocessing pipeline (no external StandardScaler needed)
5. Removed manual StandardScaler (RealMLP handles internally)
...
class CFG:
    VERSION_NAME = "v7"
    EXP_ID = "S6E4_V7_RealMLP_Baseline"
    DEVICE = DEVICE
...
# 4. MODEL PARAMETERS
# =============================================================================
# RealMLP: Residual MLP with BagEnsemble (n_ens=32 independent models)
#
# Key params mapped from S6E3 V72 (binary) → S6E4 (multiclass):
#   val_metric_name: '1-auc_ovr' → 'logloss' (always supported for multiclass)
#   n_epochs: 3 → 30 (more room for early stopping with 3 classes)
#   batch_size: 256 → 2048 (630K samples vs 7K)
#   p_drop: 0.05 → 0.1 (slightly more regularization for 3 classes)
#   ls_eps: 0.02 (kept — label smoothing helps with 3.3% minority class)
#
# tfms pipeline (applied internally by RealMLP):
#   median_center → robust_scale → smooth_clip → l2_normalize
#   NOTE: No 'one_hot' or 'embedding' (no categorical features after TE)
#
# No sample_weight (pytabkit doesn't expose it in fit())
# Class imbalance handled by: ls_eps (training) + Optuna (post-training)

REALMLP_PARAMS = {
    'random_state': CFG.RANDOM_SEED,
    'verbosity': 0,
    # Training
    'n_ens': 32,                          # 32 independent models (BagEnsemble)
    'n_epochs': 30,                       # Generous ceiling — early stopping will kick in
    'batch_size': 2048,                   # Larger for 630K samples
    'use_early_stopping': True,
    'early_stopping_additive_patience': 10,
    'early_stopping_multiplicative_patience': 1,
    # Optimizer (from S6E3 V72 proven params)
    'lr': 0.075,
    'wd': 0.0236,
    ...
    'ls_eps': 0.02,
    # Preprocessing transforms (NO 'one_hot' or 'embedding' — all numerical after TE)
    'tfms': ['median_center', 'robust_scale', 'smooth_clip', 'l2_normalize'],
}
...
if __name__ == "__main__":
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training RealMLP ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # Convert to float32 (pytabkit GPU requirement)
        X_train = X_train.fillna(0).astype('float32')
        X_val = X_val.fillna(0).astype('float32')
        X_test = X_test.fillna(0).astype('float32')

        # Train RealMLP (no sample_weight — ls_eps handles imbalance during training)
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V7 RESULTS — RealMLP Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
