"""
S6E4 V6 - LogisticRegression Baseline (CPU)
================================================================================
Strategy: LogisticRegression with Digit Features + Target Encoding + StandardScaler

Device: CPU (multi-core with n_jobs=-1)

Key advantages of LogisticRegression:
1. Linear model — captures linear relationships tree models may miss
2. Very fast training (good for diversity in ensemble)
3. StandardScaler for convergence (critical for LBFGS solver)
...
Bugs fixed from previous V6:
1. StratifiedKFold → KFold (CV overfitting with sample weights + Optuna)
2. Removed class_weight='balanced' (redundant with sample_weights — double weighting!)
"""
...
class CFG:
    VERSION_NAME = "v6"
    EXP_ID = "S6E4_V6_LogisticRegression_Baseline"
    DEVICE = "CPU"
...
LR_PARAMS = {
    ...
    'solver': 'lbfgs',               # Best for multinomial on CPU
    'multi_class': 'multinomial',    # Softmax for 3-class
    'random_state': CFG.RANDOM_SEED,
    'n_jobs': -1,                    # Parallel across classes
    'verbose': 0,
}
...
if __name__ == "__main__":
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training LogisticRegression ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        converged = "[SAVED]" if model.n_iter_[0] < LR_PARAMS['max_iter'] else "✗"
        print(f"BA: {fold_acc:.5f} | Conv: {converged} ({model.n_iter_[0]}/{LR_PARAMS['max_iter']}) | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V6 RESULTS — LogisticRegression Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
