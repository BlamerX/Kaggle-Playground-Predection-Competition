"""
S6E4 V20 - KNeighborsClassifier (K=15) Baseline (CPU)
================================================================================
Strategy: KNN with K=15 + Digit Features + Target Encoding + Class Weight Optimization

Device: CPU (multi-core with n_jobs=-1 for distance computation)

Key advantages of KNeighborsClassifier:
1. Instance-based learning — no model training, pure distance-based classification
2. Fundamentally different from all tree/linear/boosting models (V1-V19)
3. Captures local structure — can find patterns global models miss
4. Non-parametric: decision boundary adapts to local data density

Pipeline: Identical to V1-V5/V13-V19
- Digit features (8 per numerical column)
- Frequency encoding (categorical + digit columns)
- StandardScaler (MANDATORY — KNN is distance-based, scale-sensitive)
- Per-fold Target Encoding on ALL features
- KFold(10, shuffle=True, random_state=42)
- NO sample_weight — KNN doesn't support it (uses uniform/distance voting)
- Optuna class weight optimization

Key params:
- n_neighbors=15: specified by user
- weights='distance': closer neighbors have more influence (better for imbalanced data)
- algorithm='auto': lets sklearn pick best (brute force for 85 features)
- n_jobs=-1: parallelize distance computation

Speed note: KNN is prediction-heavy — stores all training data, computes distances at predict.
With 630K × 85 features, expect ~5-15 min per fold. Total: ~1-2 hours.
"""
...
# Key: StandardScaler is MANDATORY — KNN uses Euclidean distance (scale-sensitive)
# No sample_weight — KNN uses uniform/distance voting, not weight-based
KNN_PARAMS = {
    'n_neighbors': 15,                 # K=15 as specified
    'weights': 'distance',             # Closer neighbors = more influence (better for imbalance)
    'algorithm': 'auto',               # Let sklearn pick (brute for 85 dims)
    'n_jobs': -1,                      # Parallelize distance computation
}
...
if __name__ == "__main__":
    ...
    # NOTE: NO sample_weights for KNN
    # KNN uses distance-based voting, not sample weights
    # weights='distance' gives closer neighbors more influence (helps with imbalance)
    print(f"   Imbalance handling: weights='distance' (closer neighbors matter more)")
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training KNN K={KNN_PARAMS['n_neighbors']} ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # StandardScaler (MANDATORY for KNN — distance-based, scale-sensitive)
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train)
        X_val = scaler.transform(X_val)
        X_test = scaler.transform(X_test)

        # Train KNN (no sample_weight — uses distance-based voting)
        ...
        print(f"BA: {fold_acc:.5f} | K: {KNN_PARAMS['n_neighbors']} | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V20 RESULTS — KNN Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
