"""
S6E4 V42 - XGBoost DART Booster (GPU)
================================================================================
Strategy: DART (Dropout meet Multiple Additive Regression Trees) + No HPO

Device: GPU (tree_method='hist', device='cuda')

Improvements over V22 (XGB DART HPO):
1. StratifiedKFold instead of KFold (golden rule compliance)
2. NO early stopping (DART loss is non-monotonic — early stopping is INVALID)
3. subsample=1.0 (DART tree dropout already provides regularization)
4. Strong hand-tuned regularization (L1=10, L2=10, depth=4)
5. Saves 2-3 hours by skipping Optuna HPO

DART Theory:
- Standard boosting over-commits to trees early in training
- DART randomly drops trees, forcing the model to not rely on any single tree/path
- After each boosting round, dropped trees are re-integrated with normalization
- DART loss curve is NOT monotonically decreasing (trees drop in/out)
- CRITICAL: Early stopping is INVALID for DART — use fixed n_estimators
...
Speed: ~2-3 hours on GPU (Kaggle T4x2)
"""
...
try:
    ...
    print(f"[cuDF] Failed: {e}. Using standard pandas.")
...
# 4. MODEL PARAMETERS
# =============================================================================
# XGBoost DART: dropout-based boosting with regularization
# CRITICAL: NO early_stopping_rounds — DART loss is non-monotonic
XGB_DART_PARAMS = {
    ...
    'learning_rate': 0.03,        # Lower lr compensates for tree dropout
    'rate_drop': 0.3,             # 30% of trees dropped per iteration
    'skip_drop': 0.3,             # 30% chance of skipping dropout entirely
    'one_drop': 1,                # Always drop at least 1 tree
    'normalize_type': 'tree',     # Normalize by dropped tree weights
    'max_depth': 4,
    'subsample': 1.0,             # DART default: use all samples (tree dropout is enough)
...
if __name__ == "__main__":
    ...
    print(f"DART: n_estimators={XGB_DART_PARAMS['n_estimators']} (NO early stopping)")
    print("="*80)
    ...
    print(f"\n[3/6] Training XGBoost DART ({CFG.N_FOLDS}-Fold CV)...")
    print(f"   WARNING: NO early stopping — DART loss is non-monotonic")
    print(f"   Fixed {XGB_DART_PARAMS['n_estimators']} estimators per fold")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        # Target Encoding (per-fold to avoid leakage) — SAME AS V1
        ...
        # Train DART — NO early stopping!
        model = xgb.XGBClassifier(**XGB_DART_PARAMS)
        model.fit(
            X_train, y_train,
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V42 RESULTS — XGBoost DART ({CFG.DEVICE})")
    print(f"{'='*80}")
    ...
    print(f"DART: n_estimators={XGB_DART_PARAMS['n_estimators']}, rate_drop={XGB_DART_PARAMS['rate_drop']}, skip_drop={XGB_DART_PARAMS['skip_drop']}")
