"""
S6E4 V16 - DecisionTreeClassifier Baseline (CPU)
================================================================================
Strategy: DecisionTree with Digit Features + Target Encoding + Class Weight Optimization

Device: CPU (single-core, single tree — very fast)

Key advantages of DecisionTree:
1. Single tree — purest form of tree-based model (no bagging/boosting overhead)
2. Interpretable: exact decision path per prediction
3. Serves as reference point: V5 ExtraTrees = 300 random trees vs 1 optimal tree
4. Different error patterns from ensembles: single tree has high variance

Pipeline: Identical to V1-V5/V13/V14/V15
...
Key params:
- max_depth=10: single tree needs more depth than ensemble trees
- min_samples_leaf=20: regularization for 630K samples
- max_features=0.5: column subsampling for diversity
"""
...
class CFG:
    VERSION_NAME = "v16"
    EXP_ID = "S6E4_V16_DecisionTree_Baseline"
    DEVICE = "CPU"
...
# 4. MODEL PARAMETERS
# =============================================================================
# DecisionTree: single optimal tree — no bagging/boosting
# Key: needs more regularization than ensemble trees (high variance with single tree)
DT_PARAMS = {
    'max_depth': 10,                   # Deeper than ensemble trees (single tree needs capacity)
    'min_samples_split': 30,           # Regularization for 630K samples
    'min_samples_leaf': 20,            # Larger leaves to prevent overfitting
...
if __name__ == "__main__":
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training DecisionTree ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        print(f"BA: {fold_acc:.5f} | Depth: {DT_PARAMS['max_depth']} | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V16 RESULTS — DecisionTree Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
