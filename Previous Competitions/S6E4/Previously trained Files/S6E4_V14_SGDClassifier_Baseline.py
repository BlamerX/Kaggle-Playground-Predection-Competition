"""
S6E4 V14 - SGDClassifier Baseline (CPU)
================================================================================
Strategy: SGDClassifier with log_loss + StandardScaler + Digit Features + Target Encoding + Class Weight Optimization

Device: CPU (very fast — linear model, ~seconds per fold)

Key advantages of SGDClassifier:
1. NEW architecture: LINEAR model (no tree in V1-V13 uses linear boundaries)
2. loss='log_loss' = logistic regression via SGD (only loss with predict_proba)
3. OVA (One-vs-All): 3 independent binary classifiers, unique error patterns
4. Extremely fast: O(k·n·p) — 630K × 85 = seconds per fold
5. Different inductive bias: linear separator vs non-linear tree boundaries

Pipeline: Identical to V1/V2/V3/V4/V5
- Digit features (8 per numerical column)
- Frequency encoding (categorical + digit columns)
- StandardScaler (MANDATORY for SGD — linear model is scale-sensitive)
...
Key params:
- early_stopping=False: CRITICAL — score() uses accuracy not balanced_accuracy
- learning_rate='optimal': auto-scales eta = 1/(alpha * t) linked to regularization
- alpha=1e-4: moderate L2 regularization for 630K samples
- No class_weight — sample_weights handles imbalance (same as V5/V6)
"""
...
class CFG:
    VERSION_NAME = "v14"
    EXP_ID = "S6E4_V14_SGDClassifier_Baseline"
    DEVICE = "CPU"
...
# 4. MODEL PARAMETERS
# =============================================================================
# SGDClassifier: linear model optimized via stochastic gradient descent
# Key: loss='log_loss' = logistic regression (only loss with predict_proba)
# No class_weight — sample_weights handles imbalance (same pattern as V5/V6)
SGD_PARAMS = {
    'loss': 'log_loss',               # Logistic regression — only loss with predict_proba
    'penalty': 'l2',                  # L2 regularization
    'alpha': 1e-4,                    # Regularization strength
    'max_iter': 1000,                 # Sufficient for convergence
    'tol': 1e-4,                      # Convergence threshold
    'learning_rate': 'optimal',       # Auto-scales: eta = 1/(alpha * (t + t0))
    'early_stopping': False,          # CRITICAL: uses accuracy not balanced_accuracy
...
if __name__ == "__main__":
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training SGDClassifier ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # Target Encoding (per-fold to avoid leakage) — SAME AS V1/V2/V3/V4/V5
        ...
        # StandardScaler (critical for SGD — linear model is scale-sensitive)
        ...
        print(f"BA: {fold_acc:.5f} | Alpha: {SGD_PARAMS['alpha']} | Time: {fold_time:.1f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V14 RESULTS — SGDClassifier Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
