"""
S6E4 V19 - CalibratedClassifierCV (LogisticRegression + isotonic) Baseline (CPU)
================================================================================
Strategy: CalibratedClassifierCV wrapping LogisticRegression with Digit Features + Target Encoding + Class Weight Optimization

Device: CPU (multi-core with n_jobs=-1)

Key advantages of CalibratedClassifierCV:
1. Probability CALIBRATION — produces better-calibrated probabilities than raw models
2. Isotonic regression: non-parametric calibration (more flexible than sigmoid/platt scaling)
3. Different probability distribution → unique OOF patterns for hill climbing
4. Cross-validated calibration (cv=5) — fits calibrator on held-out folds
5. Based on V6 LogisticRegression — same linear model but with calibrated probabilities

Pipeline: Identical to V1-V5/V13-V18
- Digit features (8 per numerical column)
- Frequency encoding (categorical + digit columns)
- StandardScaler (mandatory for LR convergence)
...
Key params:
- Base estimator: LogisticRegression (same as V6)
- method='isotonic': non-parametric probability calibration
- cv=5: cross-validated calibration (fits on held-out data, no leakage)
- ensemble=True: average calibrated probabilities across CV folds

Note: CalibratedClassifierCV with cv>1 does NOT support sample_weight in predict_proba.
Sample_weight only affects base estimator fit (same as V6).
"""
...
class CFG:
    VERSION_NAME = "v19"
    EXP_ID = "S6E4_V19_CalibratedClassifierCV_Baseline"
    DEVICE = "CPU"
...
# 4. MODEL PARAMETERS
# =============================================================================
# LogisticRegression base (same as V6)
LR_PARAMS = {
    'C': 1.0,
    'max_iter': 2000,
    'solver': 'lbfgs',
...
# CalibratedClassifierCV: wraps LR with isotonic calibration
CAL_PARAMS = {
    'method': 'isotonic',              # Non-parametric calibration (more flexible than sigmoid)
    'cv': 5,                           # Cross-validated calibration (no leakage)
    'ensemble': True,                  # Average probabilities across calibration folds
    'n_jobs': 1,                       # Inner CV; outer loop handles parallelism
}
...
if __name__ == "__main__":
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training CalibratedClassifierCV ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # StandardScaler (critical for LR convergence)
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train)
        X_val = scaler.transform(X_val)
        X_test = scaler.transform(X_test)

        # Train CalibratedClassifierCV (LR base with isotonic calibration)
        base_lr = LogisticRegression(**LR_PARAMS)
        base_lr.fit(X_train, y_train, sample_weight=train_weights)

        model = CalibratedClassifierCV(base_lr, **CAL_PARAMS)
        ...
        print(f"BA: {fold_acc:.5f} | Method: {CAL_PARAMS['method']} | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")

        del X_train, X_val, X_test, y_train, y_val, model, base_lr, scaler, te
        gc.collect()
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V19 RESULTS — CalibratedClassifierCV Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
