"""
S6E4 V5 - ExtraTrees Baseline (CPU)
================================================================================
Strategy: ExtraTrees with Digit Features + Target Encoding + Class Weight Optimization

Device: CPU (multi-core with n_jobs=-1)

Key advantages of ExtraTrees:
1. Extremely randomized splits (faster than RandomForest, more regularization)
2. max_features for column subsampling (set to 0.6 matching LGBM)
3. No early stopping needed (bagging model — more trees = better until plateau)
4. Per-sample weights for class imbalance

Pipeline: Identical to V1/V2/V3/V4
...
Bugs fixed from previous V5:
1. StratifiedKFold → KFold (CV overfitting with sample weights + Optuna)
2. Removed class_weight='balanced' (redundant with sample_weights)
3. max_features='sqrt' → 0.6 (sqrt of 255 ≈ 16 features was way too low; 0.6 = 153 matches LGBM)
#
Speed fixes (this version):
1. n_estimators: 500 → 300 (bagging plateaus early, 300 is sufficient)
2. max_depth: 10 → 8 (less computation per tree, minimal accuracy loss)
3. min_samples_split: 12 → 20 (fewer split candidates = faster trees)
4. min_samples_leaf: 4 → 8 (larger leaves = shallower trees = faster)
5. max_features: 0.6 → 0.5 (fewer features per split = faster, more diversity)
Result: ~60-90min/fold → ~30-40min/fold (~2x faster)
"""
...
class CFG:
    VERSION_NAME = "v5"
    EXP_ID = "S6E4_V5_ExtraTrees_Baseline"
    DEVICE = "CPU"
...
# 4. MODEL PARAMETERS
# =============================================================================
# ExtraTrees: extremely randomized trees with bagging
# Key: max_features=0.6 matches LGBM feature_fraction
# No early stopping — bagging models benefit from more trees
ET_PARAMS = {
    'n_estimators': 300,               # Reduced: 500→300 (bagging plateaus early, enough trees)
    'max_depth': 8,                     # Reduced: 10→8 (less computation per tree)
    'min_samples_split': 20,            # Increased: 12→20 (fewer split candidates = faster)
    'min_samples_leaf': 8,              # Increased: 4→8 (larger leaves = shallower trees)
    'max_features': 0.5,                # Reduced: 0.6→0.5 (50% features = faster splits, more diversity)
...
# =============================================================================
# 6. FEATURE ENGINEERING (Same as V1/V2/V3/V4)
# =============================================================================
...
if __name__ == "__main__":
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training ExtraTrees ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # Target Encoding (per-fold to avoid leakage) — SAME AS V1/V2/V3/V4
        ...
        print(f"BA: {fold_acc:.5f} | Est: {ET_PARAMS['n_estimators']} | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V5 RESULTS — ExtraTrees Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
