"""
S6E4 V9 - QuadraticDiscriminantAnalysis Baseline (CPU)
================================================================================
Strategy: QDA with Digit Features + Target Encoding + StandardScaler

Device: CPU

Key advantages of QDA for Hill Climbing diversity:
1. Quadratic decision boundaries — GBDTs use axis-aligned splits, QDA uses
   elliptical class regions. Fundamentally different decision geometry.
2. Per-class covariance matrices — captures class-specific feature relationships
   that linear models (LR, Ridge) and tree models cannot.
3. No feature interactions — classifies based on multivariate Gaussian density,
   adding unique error patterns complementary to GBDTs.
4. Very fast training (~5.6 min total) — excellent diversity-per-minute.
...
- StandardScaler (critical for QDA — distance/covariance based)
- Optuna class weight optimization (post-training)

Why no sample_weight:
  QDA.fit() does not support sample_weight parameter in sklearn.
  Class imbalance handled by: Optuna weight optimization (post-training, 200 trials)

Model documentation (sklearn):
  - reg_param: Regularizes per-class covariance estimates by reducing SVD rank.
    Range [0, 1]. Higher = more regularization toward diagonal covariance.
    0.0 = full covariance (no regularization), 1.0 = maximum shrinkage.
    With 85 features and ~189K samples per class, the covariance matrix (85x85)
    is well-conditioned. reg_param=0.3 provides light regularization that
    prevents overfitting to noise while preserving quadratic boundaries.
  - store_covariance: Whether to store covariance matrix (not needed for predict).
  - tol: Threshold for rank estimation in covariance matrix.
"""
...
# Key: Each class gets its own covariance matrix → quadratic decision boundaries
# reg_param=0.3: Light regularization of covariance SVD rank
#   - Prevents singular covariance matrices in high-dimensional space
#   - With 85 features and 189K samples/class, data is sufficient but light
#     regularization helps with collinear TE-encoded features
# StandardScaler: CRITICAL — QDA uses Euclidean distances and covariance matrices,
#   which are scale-dependent. All features must be on comparable scales.
#
# priors: Balanced class priors (computed dynamically from class distribution).
#   This is the EXACT mathematical equivalent of balanced sample_weight for
#   generative models. QDA computes P(y|x) proportional to P(x|y) * P(y).
#
#   Golden rule: sample_weight = avg_count / class_count
#     - Minority class gets HIGHER weight per sample
#     - Effective class size becomes EQUAL for all classes
#
#   QDA equivalent: priors = [1/C, 1/C, 1/C]
#     - Forces P(y) = equal for all classes regardless of actual proportions
#     - Prevents majority class from dominating the posterior probability
#     - Directly aligns with Balanced Accuracy metric (equal per-class weight)
#
#   Without priors: P(y) = actual proportions (biased toward majority class)
#   With priors:    P(y) = 1/C (balanced — minority classes treated equally)
#   This is the same mathematical effect as class_weight='balanced' but for
#   generative models that compute posteriors via Bayes' theorem.

QDA_PARAMS = {
    'reg_param': 0.3,              # Covariance regularization (0=none, 1=max)
    'store_covariance': False,     # Not needed for predictions
    'tol': 1e-4,                   # Threshold for rank estimation
    'priors': [1/3, 1/3, 1/3],     # Balanced priors (equivalent to balanced sample_weight)
}
...
if __name__ == "__main__":
    ...
    # QDA does not support sample_weight in fit()
    # Using priors=[1/3, 1/3, 1/3] — mathematically equivalent to balanced sample_weight
    # for generative models (forces equal P(y) for all classes in Bayes' theorem)
    print(f"   QDA does not support sample_weight. Using balanced priors: [1/3, 1/3, 1/3]")
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training QDA ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # StandardScaler (critical for QDA — covariance-based distance)
        scaler = StandardScaler()
        X_train = scaler.fit_transform(X_train)
        X_val = scaler.transform(X_val)
        X_test = scaler.transform(X_test)

        # Train QDA (balanced priors handles class imbalance — see QDA_PARAMS)
        ...
        print(f"BA: {fold_acc:.5f} | reg_param: {QDA_PARAMS['reg_param']} | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V9 RESULTS — QDA Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
