"""
S6E4 V15 - EasyEnsembleClassifier Baseline (CPU)
================================================================================
Strategy: EasyEnsemble with Digit Features + Target Encoding + Class Weight Optimization

Device: CPU (multi-core with n_jobs=-1 across bags)

Key advantages of EasyEnsemble:
1. Bag-of-AdaBoost: N balanced bags, each trains a full AdaBoost on undersampled data
2. Each bag: RandomUnderSampler balances classes → AdaBoost focuses on hard examples
3. Probabilities averaged across all bags (inherited from BaggingClassifier)
4. Different from V13 BalancedRF: trees + bootstrap vs AdaBoost + undersampling
5. Different from V14 (skipped) AdaBoost: single AdaBoost on full data vs N AdaBoost on balanced bags

Pipeline: Identical to V1-V5/V13/V14
...
- NO sample_weight — EasyEnsemble handles imbalance via internal RandomUnderSampler
- Optuna class weight optimization

Key params:
- n_estimators=10: bags (each bag = undersampled balanced dataset + full AdaBoost)
- Inner AdaBoost: depth=3, 50 rounds (algorithm auto-handled by sklearn 1.6+)
- sampling_strategy='auto': undersamples Low/Medium to match High (~3%)
- NO sample_weight: fit() doesn't accept it — imbalance handled by undersampling only
"""
...
class CFG:
    VERSION_NAME = "v15"
    EXP_ID = "S6E4_V15_EasyEnsemble_Baseline"
    DEVICE = "CPU"
...
# 4. MODEL PARAMETERS
# =============================================================================
# EasyEnsemble: bag-of-AdaBoost with balanced undersampling per bag
# Key: each bag creates balanced data via RandomUnderSampler, then trains AdaBoost
# No sample_weight — fit() doesn't accept it, imbalance handled by undersampling only
EE_PARAMS = {
    ...
    'n_estimators': 10,                # Number of bags (each bag = balanced AdaBoost)
    'sampling_strategy': 'auto',       # Undersample Low/Medium to match High
    'replacement': False,              # No replacement in undersampling
    'n_jobs': -1,                      # Parallelize across bags
    'random_state': CFG.RANDOM_SEED,
}
...
if __name__ == "__main__":
    ...
    # NOTE: NO sample_weights for EasyEnsemble
    # EasyEnsemble handles class balancing internally via RandomUnderSampler
    # fit() does NOT accept sample_weight — imbalance handled by undersampling only
    print(f"   Imbalance handling: EasyEnsemble internal undersampling (sampling_strategy='auto')")
    ...
    # [3/6] TRAINING
    print(f"\n[3/6] Training EasyEnsemble ({CFG.N_FOLDS}-Fold CV)...")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        # Target Encoding (per-fold to avoid leakage) — SAME AS V1-V5/V13/V14
        ...
        # Train EasyEnsemble (NO sample_weight — model balances internally via undersampling)
        model = EasyEnsembleClassifier(**EE_PARAMS)
        ...
        print(f"BA: {fold_acc:.5f} | Bags: {EE_PARAMS['n_estimators']} | Time: {fold_time:.0f}s | Total: {elapsed:.1f}min")
    ...
    # [6/6] FINAL RESULTS
    print(f"\n{'='*80}")
    print(f"V15 RESULTS — EasyEnsemble Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
